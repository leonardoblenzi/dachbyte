...
assertTokenEncryptionConfigured("job diario de snapshots de reputacao ML");
...
async function main() {
  ...
  const result = force
    ? await generateDailyReputationSnapshots({
        accountIds: accountIds.length ? accountIds : null,
        onlyMissing: false,
      })
    : await runDueDailyReputationSnapshots({
        accountIds: accountIds.length ? accountIds : null,
      });
...
main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error("[Reputacao] Job diario falhou:", error?.message || error);
    process.exit(1);
  });
