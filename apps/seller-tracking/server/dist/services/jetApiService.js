...
const resolveApiBaseUrl = () => readEnv(process.env.JET_OPENAPI_BASE_URL) || 'https://openapi.plataformaneo.com.br';
const resolveApiVersion = () => readEnv(process.env.JET_OPENAPI_VERSION) || '1';
const mapJetStatusToInternal = (statusName, statusCode) => {
    const source = `${String(statusName || '')} ${String(statusCode || '')}`
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toUpperCase();
    if (source.includes('ENTREG'))
        return 'DELIVERED';
    if (source.includes('CANCEL'))
        return 'CANCELED';
    if (source.includes('DEVOL'))
        return 'RETURNED';
    if (source.includes('FALHA') || source.includes('NAO ENTREG'))
        return 'FAILURE';
    if (source.includes('ROTA') ||
...
const buildTrackingHistory = (orderPayload, mappedStatus) => {
    ...
    return sourceHistory.map((item) => {
        ...
        const statusName = pickFirstString(item?.statusExhibitionName, item?.statusName, item?.message);
        const mapped = mapJetStatusToInternal(statusName, statusCode);
        return {
            status: mapped,
            description: pickFirstString(item?.message, item?.statusExhibitionName, item?.statusName) ||
...
const unwrapJetOrderResult = (payload) => {
...
class JetApiService {
    ...
    async getClient() {
        ...
        if (!configuration.integrationKey) {
            throw new Error('JET sem credencial apiKey configurada. Informe a Credencial de Integracao JET.');
        }
        const headers = {
        ...
        if (configuration.bearerToken) {
            ...
            const encoded = Buffer.from(`${configuration.username || ''}:${configuration.password || ''}`, 'utf8').toString('base64');
    ...
    async getOrderById(idOrder) {
        ...
        const path = `/order/api/v${encodeURIComponent(configuration.apiVersion)}/id/${encodeURIComponent(id)}`;
        try {
    ...
    mapJetOrderToSystem(jetOrder) {
        ...
        const invoiceNumber = pickFirstString(order?.billNumber, historyOrderInfo?.noteNumber);
        const trackingCode = pickFirstString(historyOrderInfo?.trackingNumber, order?.codigoExternoFrete, order?.shippingRegister);
        const trackingUrl = pickFirstString(latestHistory?.trackingLink, order?.trackingLink);
        const invoiceAccessKey = pickFirstString(historyOrderInfo?.key, order?.proofOfSale);
        const freightType = pickFirstString(order?.nameCarrying, order?.shippingCompany, order?.nameShipping);
        ...
        const shippingDate = safeDate(order?.dateOrder) ||
...
exports.JetApiService = JetApiService;
