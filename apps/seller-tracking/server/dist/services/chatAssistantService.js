...
const normalizeText = (value) => String(value || '')
    ...
    .replace(/\s+/g, ' ')
    .trim();
const getMatchTokens = (value) => normalizeText(value)
    .split(' ')
    .map((token) => token.trim())
    .filter((token) => token.length >= 3 && !OPTION_MATCH_STOPWORDS.has(token));
const scoreOptionMatch = (option, normalizedInput) => {
    ...
    for (const optionToken of optionTokens) {
        const matchedToken = inputTokens.find((inputToken) => inputToken === optionToken ||
        ...
        sharedTokens += 1;
        longestSharedTokenLength = Math.max(longestSharedTokenLength, Math.min(matchedToken.length, optionToken.length));
    }
    if (sharedTokens > 0) {
...
const hasStatusHint = (normalized) => STATUS_MATCHERS.some((matcher) => matcher.phrases.some((phrase) => normalized.includes(phrase)));
const hasDelayHint = (normalized) => normalized.includes('atras') ||
    normalized.includes('em atraso') ||
    normalized.includes('transportadora') ||
    normalized.includes('plataforma');
const hasGenericDelayTerm = (normalized) => normalized.includes('atrasado') ||
    normalized.includes('atrasados') ||
    normalized.includes('em atraso') ||
    normalized.includes('pedidos atrasados');
const hasExplicitPlatformDelayHint = (normalized) => normalized.includes('atraso plataforma') ||
...
const hasExplicitCarrierDelayHint = (normalized) => normalized.includes('atraso transportadora') ||
...
const isAmbiguousDelayRequest = (normalized, carrierName) => hasGenericDelayTerm(normalized) &&
    !hasExplicitPlatformDelayHint(normalized) &&
    !hasExplicitCarrierDelayHint(normalized) &&
    !carrierName;
const hasNoMovementHint = (normalized) => normalized.includes('sem movimentacao') ||
    normalized.includes('sem movimento') ||
    normalized.includes('sem atualizacao');
const hasPeriodHint = (normalized) => normalized.includes('hoje') ||
    normalized.includes('ontem') ||
    /(?:nos\s+)?(?:ultimos|uiltimos|ultimas|ultimas)\s+\d+\s*dias?/.test(normalized);
const hasOperationalContextHint = (normalized) => normalized.includes('pedido') ||
...
const hasFilterLikeIntent = (normalized) => (hasStatusHint(normalized) || hasDelayHint(normalized) || hasNoMovementHint(normalized)) &&
...
const hasStatusSummaryHint = (normalized) => normalized.includes('por status') ||
...
const buildIntelipostTrackingUrl = (clientId, orderNumber) => {
...
const looksLikeXmlIdentifier = (value) => {
...
const extractTrackingLookupRequest = (text) => {
    ...
    const customerMatch = rawText.match(/(?:do|da|de)?\s*cliente\s+(.+)$/i);
    ...
    const tokenCandidates = rawText.match(/[A-Za-z]{2}\d{9}[A-Za-z]{2}|\d{4,}|[A-Za-z0-9-]{20,}/g);
...
const mapTrackingEventsToHistory = (trackingEvents) => Array.isArray(trackingEvents)
    ...
    : [];
const getStoredTrackingUrl = (order) => safeString(order.trackingUrl) ||
...
const mapIntelipostStatusToOrderStatus = (value) => {
...
const getReportsDir = () => path_1.default.join(__dirname, '../../public/reports/chat-insights');
...
const extractOrderIdentifiers = (text, stopwords) => {
...
const isMonitoredOrderIncludeIntent = (text) => {
    const normalized = normalizeText(text);
    const hasActionVerb = normalized.includes('inclua') ||
    ...
    const hasTargetContext = normalized.includes('pedido monitorado') ||
...
const extractMonitoredIdentifiers = (text) => {
    return extractOrderIdentifiers(text, MONITORED_IDENTIFIER_STOPWORDS);
};
const extractArchiveIdentifiers = (text) => extractOrderIdentifiers(text, ARCHIVE_IDENTIFIER_STOPWORDS);
const isArchiveIntent = (text) => {
    const normalized = normalizeText(text);
    return (!normalized.includes('desarquiv') &&
...
const isDeleteByCarrierIntent = (text) => {
    const normalized = normalizeText(text);
    const hasDeleteVerb = normalized.includes('excluir') ||
    ...
    const hasOrderContext = normalized.includes('pedido') || normalized.includes('pedidos');
    const hasCarrierContext = normalized.includes('transportadora') || normalized.includes('transportadoras');
    return hasDeleteVerb && hasOrderContext && hasCarrierContext;
};
const isArchiveByCarrierIntent = (text) => {
    const normalized = normalizeText(text);
    if (normalized.includes('desarquiv')) {
        return false;
    }
    const hasArchiveVerb = normalized.includes('arquivar') || normalized.includes('arquive');
    const hasOrderContext = normalized.includes('pedido') || normalized.includes('pedidos');
    const hasCarrierContext = normalized.includes('transportadora') || normalized.includes('transportadoras');
...
const extractOrderUpdateIdentifiers = (text) => extractOrderIdentifiers(text, ORDER_UPDATE_IDENTIFIER_STOPWORDS);
const isManualOrderStatusUpdateIntent = (text) => {
    const normalized = normalizeText(text);
...
const resolveManualOrderStatus = (text) => {
    const normalized = normalizeText(text);
    const targetMatch = normalized.match(/(?:status(?:\s+dos?\s+pedidos?)?\s+(?:para|como)|(?:para|como)\s+status)\s+(.+)/);
    const target = targetMatch?.[1] || '';
    if (/\b(saiu para entrega|em rota)\b/.test(target))
        return orderStatus_1.OrderStatus.DELIVERY_ATTEMPT;
    if (/\b(entregue|concluido|concluida|concluidos|concluidas)\b/.test(target))
        return orderStatus_1.OrderStatus.DELIVERED;
    if (/\b(falha|falhou|falhado)\b/.test(target))
        return orderStatus_1.OrderStatus.FAILURE;
    if (/\b(devolvido|devolvida|devolvidos|devolvidas)\b/.test(target))
        return orderStatus_1.OrderStatus.RETURNED;
    if (/\b(cancelado|cancelada|cancelados|canceladas)\b/.test(target))
        return orderStatus_1.OrderStatus.CANCELED;
    if (/\b(pendente|aguardando envio)\b/.test(target))
        return orderStatus_1.OrderStatus.PENDING;
    if (/\b(criado|criada)\b/.test(target))
        return orderStatus_1.OrderStatus.CREATED;
    if (/\b(enviado|enviada|despachado|despachada|em transito)\b/.test(target))
        return orderStatus_1.OrderStatus.SHIPPED;
    return null;
};
const isCompleteOrderIntegrationRefreshIntent = (text) => {
    const normalized = normalizeText(text);
...
const isTrackingSyncByStatusIntent = (text) => {
    const normalized = normalizeText(text);
    const hasSyncVerb = normalized.includes('atualizar') ||
        normalized.includes('atualize') ||
        normalized.includes('sincronizar') ||
        normalized.includes('sincronize') ||
        normalized.includes('sync');
    const hasTrackingContext = normalized.includes('rastreio') || normalized.includes('tracking');
...
const isUnarchiveIntent = (text) => {
    const normalized = normalizeText(text);
    return (normalized.includes('desarquivar') ||
        normalized.includes('desarquive') ||
        normalized.includes('retirar do arquivo') ||
        normalized.includes('tirar do arquivo'));
};
const hasBulkArchiveHint = (text) => {
    const normalized = normalizeText(text);
    return (normalized.includes('todos os pedidos') ||
...
const hasActionableBulkArchiveFilter = (filter) => Boolean(filter &&
    (filter.status ||
        filter.delayedKind ||
        filter.noMovementDays ||
        filter.carrierName ||
        filter.salesChannel));
const inferMonitoredWatchEvents = (text) => {
    const normalized = normalizeText(text);
    if (normalized.includes('todos os status') ||
    ...
    if (normalized.includes('mudanca de status') ||
...
const describeWatchEvents = (watchEvents) => {
...
const resolveIntentKind = (text) => {
    const normalized = normalizeText(text);
    if (normalized.includes('relatorio') ||
...
const resolvePeriodFilter = (text) => {
    const normalized = normalizeText(text);
    const now = new Date();
    ...
    const lastDaysMatch = normalized.match(/(?:(?:nos|das|ha)\s+)?(?:ultimos|uiltimos|ultimas)\s+(\d+)\s*dias?/);
    if (lastDaysMatch) {
...
const buildStatusEventSqlClause = (status) => {
    if (status === orderStatus_1.OrderStatus.DELIVERED) {
        return `
      (
    ...
    if (status === orderStatus_1.OrderStatus.SHIPPED) {
        return `
      (
    ...
    if (status === orderStatus_1.OrderStatus.DELIVERY_ATTEMPT) {
        return `
      (
    ...
    if (status === orderStatus_1.OrderStatus.FAILURE) {
        return `
      (
...
const buildOrderWhereSql = (companyId, filter, options = {}) => {
    const now = new Date();
    const params = [companyId, options.archived ?? false];
    const conditions = ['o."companyId" = $1', 'o."isArchived" = $2'];
...
const buildReportHtml = (input) => `<!DOCTYPE html>
<html lang="pt-BR">
  <head>
    <meta charset="UTF-8" />
...
const getConversationMessageText = (message) => String(message?.text || message?.content || '').trim();
const getPreviousUserMessageText = (messages, currentText) => {
...
const buildContextAwareStructuredInput = (text, messages) => {
    ...
    const assistantMessages = Array.isArray(messages)
        ...
        : [];
    const lastAssistantText = normalizeText(getConversationMessageText(assistantMessages[assistantMessages.length - 1]));
...
class ChatAssistantService {
    ...
    pendingCarrierDeleteOperations = new Map();
    trackingStatusSyncJobs = new Map();
    static PENDING_DELETE_TTL_MS = 15 * 60 * 1000;
    buildPendingCarrierDeleteKey(companyId, userId) {
        return `${companyId}:${userId}`;
    }
    pruneExpiredPendingCarrierDeleteOperations() {
        const now = Date.now();
    ...
    extractCarrierTermsFromText(text) {
        const rawText = String(text || '').trim();
        if (!rawText)
            return [];
    ...
    async resolveCarrierNamesFromText(companyId, text) {
    ...
    async countOrdersByCarriers(companyId, carriers) {
        if (!carriers.length)
            return [];
      ...
      `, [companyId, carriers]);
        const rowsByCarrier = new Map(rowsResult.rows
            .map((row) => [String(row.carrierName || '').trim(), Number(row.total || 0)])
    ...
    async archiveOrdersByCarriers(companyId, carriers) {
        if (!carriers.length) {
            return {
                updatedByCarrier: [],
      ...
      `, [companyId, carriers]);
        ...
        const updatedByCarrierMap = new Map();
        for (const row of updatedRows) {
            const carrierName = String(row.carrierName || '').trim();
            if (!carrierName)
                continue;
            updatedByCarrierMap.set(carrierName, (updatedByCarrierMap.get(carrierName) || 0) + 1);
        }
    ...
    async deleteOrdersByCarriers(companyId, carriers) {
        if (!carriers.length) {
            return [];
        }
        const deletedResult = await (0, db_1.dbQuery)(`
        DELETE FROM "Order" o
        WHERE o."companyId" = $1
          AND o."freightType" = ANY($2::text[])
        RETURNING o."freightType" AS "carrierName"
      `, [companyId, carriers]);
        const deletedByCarrierMap = new Map();
        for (const row of deletedResult.rows) {
            const carrierName = String(row.carrierName || '').trim();
            if (!carrierName)
                continue;
            deletedByCarrierMap.set(carrierName, (deletedByCarrierMap.get(carrierName) || 0) + 1);
        }
        return carriers.map((carrierName) => ({
            carrierName,
    ...
    formatCarrierBreakdown(entries, action) {
        return entries
            .map((item) => {
    ...
    async handlePendingCarrierDeleteConfirmation(input) {
        ...
        this.pendingCarrierDeleteOperations.delete(key);
        const deletedByCarrier = await this.deleteOrdersByCarriers(input.companyId, operation.carriers);
        const totalDeleted = deletedByCarrier.reduce((accumulator, item) => accumulator + item.total, 0);
        if (totalDeleted === 0) {
            return {
                handled: true,
        ...
        return {
            handled: true,
            text: `Exclusao concluida com sucesso. Foram removidos ${totalDeleted} pedidos do banco: ${this.formatCarrierBreakdown(deletedByCarrier, 'excluir')}.`,
        };
    }
    buildTrackingSummary(order) {
        ...
        const latestLocation = latestEvent && (latestEvent.city || latestEvent.state)
    ...
    buildCustomerDisambiguation(customerName, orders) {
        return [
            `Encontrei mais de um pedido para o cliente ${customerName}.`,
            '',
            ...orders.slice(0, 5).map((order, index) => `${index + 1}. Pedido ${order.orderNumber} | NF ${order.invoiceNumber || '-'} | ${STATUS_LABELS[order.status] || order.status} | Ultima mov. ${formatDate(order.lastUpdate)}`),
            '',
            'Se quiser, me peça por numero do pedido, NF ou XML para eu trazer o rastreio exato.',
        ].join('\n');
    }
    buildExternalOrderPayload(request, source, result) {
        if (source === 'SSW') {
            ...
            const recipientName = safeString(result.matchMetadata?.recipientName) ||
                ...
                'Consulta externa';
            const documentFields = splitDocumentFields(safeString(result.matchMetadata?.deliveredToDocument) ||
            ...
            return {
                ...
                invoiceNumber: String(result.lookupMode || '').toUpperCase() === 'INVOICE'
                    ? normalizeDigits(request.value)
                    : null,
                trackingCode: String(result.lookupMode || '').toUpperCase() === 'XML_KEY'
        ...
        const trackingHistory = Array.isArray(result?.tracking?.history)
            ? result.tracking.history.map((historyItem) => ({
                status: safeString(historyItem?.macro_state?.code) || 'UNKNOWN',
                description: safeString(historyItem?.provider_message) ||
        ...
        const trackingUrl = buildIntelipostTrackingUrl(safeString(result?.client?.id), resolvedOrderNumber);
        return {
    ...
    async fetchTrackingOrderById(orderId) {
      ...
      `, [orderId]);
        ...
        const trackingEventsResult = await (0, db_1.dbQuery)(`
        SELECT
          te."status",
          te."description",
          te."city",
    ...
    async tryHandleTrackingRequest(input) {
        ...
        if (request.kind === 'customer') {
            ...
            const matchesResult = await (0, db_1.dbQuery)(`
          SELECT
            o."id",
            o."orderNumber",
            o."invoiceNumber",
        ...
        const localMatchesResult = await (0, db_1.dbQuery)(`
        SELECT
          o."id",
          o."lastUpdate"
        FROM "Order" o
      ...
      `, [
        ...
        const externalResult = await this.trackingService.searchExternalIdentifier(request.value, input.companyId);
        if (!externalResult) {
            return {
                handled: true,
        ...
        const externalOrderPayload = this.buildExternalOrderPayload(request, externalResult.source, externalResult.result);
        await (0, orderImportService_1.importOrdersForCompany)(input.companyId, [externalOrderPayload]);
        const savedOrderResult = await (0, db_1.dbQuery)(`
        SELECT o."id"
        FROM "Order" o
        WHERE o."companyId" = $1
          AND o."orderNumber" = $2
      ...
      `, [input.companyId, String(externalOrderPayload.orderNumber)]);
        ...
        return {
            ...
            text: orderForSummary
                ? shouldReturnLink
                    ? this.buildTrackingLinkReply(orderForSummary)
                    : this.buildTrackingSummary(orderForSummary)
                : `Nao consegui montar o rastreio de ${request.label} ${request.value} agora.`,
        };
    }
    async buildCarrierStatusSummary(companyId, filter) {
        if (!filter.carrierName &&
            !filter.salesChannel &&
        ...
        const statusRowsResult = await (0, db_1.dbQuery)(`
        SELECT
          o."status" AS status,
          COUNT(*)::int AS total
        FROM "Order" o
        ...
        GROUP BY o."status"
      `, baseWhere.params);
        const countsByStatus = new Map(statusRowsResult.rows.map((row) => [row.status, Number(row.total || 0)]));
        const carrierWhere = buildOrderWhereSql(companyId, {
            ...summaryBaseFilter,
            delayedKind: 'carrier',
        });
        const carrierDelayedResult = await (0, db_1.dbQuery)(`
      ...
      `, carrierWhere.params);
        ...
        const platformDelayedResult = await (0, db_1.dbQuery)(`
        SELECT COUNT(*)::int AS total
        FROM "Order" o
      ...
      `, platformWhere.params);
        ...
        return [
            ...
            ...STATUS_SUMMARY_ORDER.map((status) => `- ${STATUS_LABELS[status] || status}: ${countsByStatus.get(status) || 0}`),
        ].join('\n');
    }
    async resolveExactCarrierName(companyId, normalizedInput) {
        const carriersResult = await (0, db_1.dbQuery)(`
    ...
    async resolveExactMarketplaceName(companyId, normalizedInput) {
        const channelsResult = await (0, db_1.dbQuery)(`
    ...
    async resolveOrdersByIdentifiers(input) {
    ...
    async getEnabledOrderIntegrations(companyId) {
        const result = await (0, db_1.dbQuery)(`
        SELECT
      ...
      `, [companyId]);
        ...
        if (flags.trayIntegrationEnabled !== false)
            integrations.push('tray');
        if (flags.magazordIntegrationEnabled === true)
            integrations.push('magazord');
        if (flags.anymarketIntegrationEnabled === true)
            integrations.push('anymarket');
        if (flags.jetIntegrationEnabled === true)
            integrations.push('jet');
        return integrations;
    }
    async fetchOrderFromIntegration(integration, companyId, orderNumber) {
        if (integration === 'tray') {
    ...
    async updateOrdersFromIntegration(input) {
        ...
        for (const order of resolution.foundOrders) {
            ...
            for (const integration of integrations) {
                try {
                    const mappedOrder = await this.fetchOrderFromIntegration(integration, input.companyId, String(order.orderNumber));
                    if (!mappedOrder) {
                        attempts.push(`${integration}: pedido nao localizado`);
                        continue;
                    }
                    const result = await (0, orderImportService_1.importOrdersForCompany)(input.companyId, [mappedOrder]);
            ...
            if (updated)
                ...
                failedOrders.push(`${label}${attempts.length ? ` (${attempts.join('; ')})` : ''}`);
        }
        return { ...resolution, updatedOrders, skippedArchived, failedOrders, integrations };
    }
    async updateOrdersManualStatus(input) {
        const resolution = await this.resolveOrdersByIdentifiers(input);
        const updatedOrders = [];
        const skippedArchived = [];
    ...
    async setOrderArchiveByIdentifiers(input) {
        const normalizedIdentifiers = Array.from(new Set((Array.isArray(input.identifiers) ? input.identifiers : [])
        ...
        const alreadyInTargetState = foundOrders.filter((order) => order.isArchived === input.archived);
        ...
        const toLabel = (order) => order.invoiceNumber
            ? `Pedido ${order.orderNumber} / NF ${order.invoiceNumber}`
            : `Pedido ${order.orderNumber}`;
        return {
    ...
    async setOrderArchiveByFilter(input) {
        const updateFilter = buildOrderWhereSql(input.companyId, input.filter, {
            archived: !input.archived,
        });
        const ordersToUpdateResult = await (0, db_1.dbQuery)(`
        SELECT o."id"
        FROM "Order" o
      ...
      `, updateFilter.params);
        ...
        const alreadyCountResult = await (0, db_1.dbQuery)(`
        SELECT COUNT(*)::int AS total
        FROM "Order" o
    ...
    async runTrackingSyncByStatusJob(input) {
    ...
    async resolveFilters(companyId, input) {
        ...
        if (normalized.includes('sem movimentacao') ||
            normalized.includes('sem movimento') ||
            normalized.includes('sem atualizacao')) {
        ...
        if (normalized.includes('todos os pedidos') ||
    ...
    async tryHandleStructuredRequest(input) {
        if (input.companyId && input.userId) {
            const pendingConfirmationResult = await this.handlePendingCarrierDeleteConfirmation({
      ...
      `, [input.companyId]);
        ...
        if (isManualOrderStatusUpdateIntent(input.text)) {
            ...
            if (result.skippedArchived.length > 0)
                lines.push(`Arquivados e nao alterados: ${result.skippedArchived.join(', ')}.`);
            if (result.notFoundIdentifiers.length > 0)
                lines.push(`Nao encontrados: ${result.notFoundIdentifiers.join(', ')}.`);
            return {
                handled: true,
        ...
        if (isCompleteOrderIntegrationRefreshIntent(input.text)) {
            ...
            const lines = result.updatedOrders.length > 0
                ...
                : [];
            if (result.skippedArchived.length > 0)
                lines.push(`Arquivados e nao atualizados: ${result.skippedArchived.join(', ')}.`);
            if (result.notFoundIdentifiers.length > 0)
                lines.push(`Nao encontrados: ${result.notFoundIdentifiers.join(', ')}.`);
            if (result.failedOrders.length > 0)
                lines.push(`Sem atualizacao: ${result.failedOrders.join(' | ')}.`);
            return {
                handled: true,
        ...
        if (isTrackingSyncByStatusIntent(input.text)) {
            ...
            const wantsDelayedSync = !syncFilter?.status &&
                (Boolean(syncFilter?.delayedKind) || hasGenericDelayTerm(normalizedSyncText));
            if (!syncFilter?.status && !wantsDelayedSync) {
                return {
                    handled: true,
                    text: 'Para atualizar o rastreio por status, informe tambem o status. Exemplo: atualizar o rastreio dos pedidos com status entregue ou status atrasado.',
                };
            }
            let syncKey = '';
            let syncLabel = '';
            ...
            if (syncFilter?.status) {
                if (syncFilter.status === orderStatus_1.OrderStatus.CANCELED) {
                    return {
                        handled: true,
                ...
                const ordersToSyncResult = await (0, db_1.dbQuery)(`
            SELECT o."id"
            FROM "Order" o
            WHERE o."companyId" = $1
              AND o."status" = $2
              ...
              AND o."status" <> $3
            ORDER BY o."lastUpdate" DESC
          `, [company.id, syncFilter.status, orderStatus_1.OrderStatus.CANCELED]);
                ...
                const delayedOrdersResult = await (0, db_1.dbQuery)(`
            SELECT o."id"
            FROM "Order" o
            WHERE o."companyId" = $1
              AND o."isArchived" = FALSE
        ...
        if (isDeleteByCarrierIntent(input.text)) {
            ...
            const { resolvedCarriers, unresolvedTerms } = await this.resolveCarrierNamesFromText(company.id, input.text);
            if (resolvedCarriers.length === 0) {
                return {
                    handled: true,
                    text: unresolvedTerms.length > 0
            ...
            const countsByCarrier = await this.countOrdersByCarriers(company.id, resolvedCarriers);
            const totalToDelete = countsByCarrier.reduce((accumulator, item) => accumulator + item.total, 0);
            if (totalToDelete === 0) {
                return {
                    handled: true,
            ...
            const unresolvedNotice = unresolvedTerms.length > 0
                ? `\nNao reconheci estas entradas e elas serao ignoradas: ${unresolvedTerms.join(', ')}.`
                : '';
            return {
                handled: true,
                text: `Irei excluir ${this.formatCarrierBreakdown(countsByCarrier, 'excluir')}. Tem certeza?${unresolvedNotice}`,
                actions: [
                    { label: 'Sim', value: 'sim', style: 'danger' },
                    { label: 'Nao', value: 'nao', style: 'secondary' },
                ],
        ...
        if (isArchiveByCarrierIntent(input.text)) {
            ...
            const archiveResult = await this.archiveOrdersByCarriers(company.id, resolvedCarriers);
            const totalArchived = archiveResult.updatedByCarrier.reduce((accumulator, item) => accumulator + item.total, 0);
            const lines = [
                totalArchived > 0
            ...
            const alreadyTotal = archiveResult.alreadyArchivedByCarrier.reduce((accumulator, item) => accumulator + item.total, 0);
            if (alreadyTotal > 0) {
                lines.push(`Ja estavam arquivados: ${this.formatCarrierBreakdown(archiveResult.alreadyArchivedByCarrier, 'arquivar')}.`);
            }
            if (unresolvedTerms.length > 0) {
                lines.push(`Nao reconheci estas entradas e elas foram ignoradas: ${unresolvedTerms.join(', ')}.`);
            }
            return {
                handled: true,
                text: lines.join('\n'),
        ...
        if (isArchiveIntent(input.text) || isUnarchiveIntent(input.text)) {
            ...
            if (bulkArchiveRequested) {
                ...
                if (hasActionableBulkArchiveFilter(bulkFilter)) {
                    ...
                    const lines = archiveResult.updatedCount > 0
                        ? [
                    ...
                    if (archiveResult.alreadyInTargetStateCount > 0) {
                        lines.push(`${archiveResult.alreadyInTargetStateCount} ${filterLabel} ${alreadyLabel}.`);
                    }
                    return {
                        handled: true,
                        text: lines.join('\n'),
            ...
            const alreadyLabels = archiveResult.alreadyInTargetState.map((item) => item.label);
            const lines = [];
            if (updatedLabels.length > 0) {
                lines.push(shouldArchive
            ...
            if (archiveResult.notFoundIdentifiers.length > 0) {
                lines.push(`Nao encontrados: ${archiveResult.notFoundIdentifiers.join(', ')}.`);
            }
            return {
                handled: true,
                text: lines.join('\n') ||
        ...
        if (isMonitoredOrderIncludeIntent(input.text)) {
            ...
            if (identifiers.length === 0) {
                return {
                    handled: true,
                    text: 'Para incluir pedidos monitorados, me envie os numeros dos pedidos ou NFs. Exemplo: inclua os pedidos 123, 456 ou inclua as NFs 9981, 9982.',
                };
            }
            ...
            const limitExceededLabels = Array.isArray(result.limitExceededOrders)
                ? result.limitExceededOrders.map((item) => item.label)
                : [];
            ...
            if (addedLabels.length > 0) {
                ...
                if (addedOrderIds.length > 0) {
                    ...
                    WHERE u."id" = $1
                    LIMIT 1
                  `, [input.userId])).rows[0] || null
                        : null;
                    try {
                        await notificationService_1.notificationService.sendMonitoredEnrollmentEmail({
                            companyId: company.id,
                            orderIds: addedOrderIds,
                            watchEvents,
                            requestedByName: requestedBy?.name || null,
                        });
                        lines.push('Enviei um e-mail de confirmacao do monitoramento com os dados dos pedidos.');
                    }
                    catch (emailError) {
                        console.error('Falha ao enviar e-mail de confirmacao do monitoramento:', emailError);
                        lines.push('Nao consegui enviar o e-mail de confirmacao agora, mas o monitoramento foi ativado.');
                    }
                }
                if (alreadyLabels.length > 0) {
                    lines.push(`Ja monitorados: ${alreadyLabels.join(', ')}.`);
                }
                if (result.notFoundIdentifiers.length > 0) {
                    lines.push(`Nao encontrados: ${result.notFoundIdentifiers.join(', ')}.`);
                }
                if (limitExceededLabels.length > 0) {
                    lines.push(`Limite de ${maxMonitoredOrders} monitorados por empresa atingido. Sem vaga para: ${limitExceededLabels.join(', ')}.`);
                }
                return {
                    handled: true,
                    text: lines.join('\n'),
            ...
            return {
                handled: true,
                text: limitExceededLabels.length > 0
        ...
        const effectiveText = buildContextAwareStructuredInput(input.text, input.messages);
        const filter = await this.resolveFilters(company.id, effectiveText);
        if (!filter) {
            const normalizedInput = normalizeText(effectiveText);
            const inferredCarrier = await this.resolveExactCarrierName(company.id, normalizedInput);
        ...
        const countResult = await (0, db_1.dbQuery)(`
        SELECT COUNT(*)::int AS total
        FROM "Order" o
      ...
      `, where.params);
        ...
        await Promise.all([
            promises_1.default.writeFile(path_1.default.join(reportsDir, `${reportId}.html`), buildReportHtml({
            ...
            promises_1.default.writeFile(path_1.default.join(reportsDir, `${reportId}.csv`), buildReportCsv(orders), 'utf8'),
        ]);
        const carrierStatusSummary = await this.buildCarrierStatusSummary(company.id, filter);
        return {
            handled: true,
            text: [
...
exports.chatAssistantService = new ChatAssistantService();
