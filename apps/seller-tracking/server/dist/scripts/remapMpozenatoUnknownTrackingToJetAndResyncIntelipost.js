...
const APPLY_CHANGES = String(process.env.APPLY_CHANGES || '')
    .trim()
    .toLowerCase() === 'true';
const ONLY_ORDER_NUMBERS = String(process.env.ONLY_ORDER_NUMBERS || '326136837,326315748')
    .split(',')
    .map((item) => String(item || '').trim())
    .filter(Boolean);
const JET_API_KEY = String(process.env.JET_API_KEY || '').trim();
const JET_BASE_URL = String(process.env.JET_BASE_URL || 'https://openapi.plataformaneo.com.br')
    .trim()
    .replace(/\/+$/, '');
...
const parseOrderNumberIdMap = () => {
    ...
    try {
        ...
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
            return new Map(Object.entries(parsed)
                .map(([from, to]) => [String(from).trim(), normalizeDigits(to)])
    ...
    return new Map(rawValue
        ...
        return [String(from || '').trim(), normalizeDigits(to)];
    })
        .filter(([from, to]) => from.length > 0 && to.length > 0));
};
...
const fetchJetOrderById = async (idOrder, apiKey) => {
    const normalizedId = normalizeDigits(idOrder);
    if (!normalizedId) {
        return null;
    }
    const url = `${JET_BASE_URL}/order/api/v${encodeURIComponent(JET_API_VERSION)}/id/${encodeURIComponent(normalizedId)}`;
...
const resolveJetIdForOrder = async (order, manualMap, jetApiKey) => {
    const orderNumber = String(order.orderNumber || '').trim();
    const rawPayload = order.apiRawPayload || {};
    const fromPayload = normalizeDigits(rawPayload?.jetOrderId) ||
...
async function main() {
    ...
    for (const order of targetOrders) {
        ...
        const rawPayload = order.apiRawPayload && typeof order.apiRawPayload === 'object'
            ...
            : {};
        console.log('\n---');
        console.log(`Pedido ${currentOrderNumber} | id=${order.id} | arquivado=${order.isArchived ? 'SIM' : 'NAO'} | ultimo="${order.latestTrackingDescription || '-'}"`);
        ...
        if (targetJetId === currentOrderNumber) {
            ...
            const conflictResult = await (0, db_1.dbQuery)(`
          SELECT o."id"
          FROM "Order" o
          WHERE o."companyId" = $1
            AND o."orderNumber" = $2
        ...
        `, [company.id, targetJetId, order.id]);
            if (conflictResult.rows[0]) {
                conflicts += 1;
                console.log(`Conflito: ja existe outro pedido com orderNumber ${targetJetId}.`);
                continue;
            }
            const nextPayload = {
                ...rawPayload,
            ...
            swapped += 1;
            console.log(`${APPLY_CHANGES ? 'Atualizado' : 'Planejado'}: ${currentOrderNumber} -> ${targetJetId} (${resolvedJet.source}).`);
        }
    ...
    if (!APPLY_CHANGES) {
        ...
        console.log('Se algum pedido ficar "Sem idOrder resolvido", informe ORDER_NUMBER_ID_MAP no formato JSON ou "order:idOrder".');
    }
}
main().catch((error) => {
