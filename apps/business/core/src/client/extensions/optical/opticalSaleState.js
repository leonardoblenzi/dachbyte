...
function opticalProductExtension(product = {}) {
...
function filterOpticalSaleProducts(products, saleFilter, productQuery) {
  ...
  return products
    ...
    .filter((item) => {
      ...
      return normalizeSearchText(`${item.name} ${item.sku} ${item.ean} ${item.category} ${item.brand} ${item.type} ${opticalProductExtension(item).opticalType || ""}`).includes(query);
    });
}

function isOpticalSaleCart(cart) {
  return cart.some((item) => opticalProductExtension(item).opticalType || /armacao|lente|otico/i.test(`${item.category} ${item.name}`.normalize("NFD").replace(/[\u0300-\u036f]/g, "")));
}
...
function deriveOpticalSaleState({ data, draft, productQuery, saleFilter }) {
  ...
  const isPaymentOnDelivery = draft.paymentTiming === "delivery";
...
function validateOpticalSale({ draft, customer, optical, paymentTotal, total, remaining, change, mode }) {
  ...
  if (draft.paymentTiming === "delivery") {
    if (!customer) return "Pagamento na entrega exige cliente cadastrado.";
    if (!draft.promisedDate) return "Informe a previsao de entrega para criar o pedido.";
    return "";
  }
...
function buildPrescriptionPayload(prescription = {}) {
  ...
  const sharedAddition = addition || rightAddition || leftAddition || null;

  return {
    ...distancePrescription,
    rightAddition: sharedAddition,
    leftAddition: sharedAddition,
    metadata: {
      ...(distancePrescription.metadata || {}),
      near: {
        right: {
          spherical: nearRightSpherical || null,
          cylindrical: nearRightCylindrical || null,
          axis: nearRightAxis || null,
        },
        left: {
          spherical: nearLeftSpherical || null,
          cylindrical: nearLeftCylindrical || null,
          axis: nearLeftAxis || null,
        },
      },
    },
  };
...
function buildOpticalSaleSubmitPayload({ draft, customer, discount, optical, payments }) {
  return {
    ...
    paymentTiming: draft.paymentTiming || "immediate",
    promisedDeliveryDate: draft.paymentTiming === "delivery" ? draft.promisedDate : null,
    payments: draft.paymentTiming === "delivery" ? [] : payments,
    ...
    ...(optical ? {
      extensions: {
        "vertical.optical": {
          ...
          prescription: draft.prescriptionMode === "now" ? buildPrescriptionPayload(draft.prescription) : null,
          ...
          frameProductId: draft.cart.find((item) => opticalProductExtension(item).opticalType === "frame")?.id || null,
          lensProductId: draft.cart.find((item) => opticalProductExtension(item).opticalType === "lens")?.id || null,
          notes: draft.notes,
        },
      },
    } : {}),
