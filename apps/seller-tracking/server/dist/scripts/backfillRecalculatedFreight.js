...
const trayFreightService_1 = require("../services/trayFreightService");
const PAGE_SIZE = 100;
const formatError = (error) => error instanceof Error ? error.message : 'Erro desconhecido';
async function processCompany(company) {
    console.log(`\nEmpresa: ${company.name}`);
    const freightService = new trayFreightService_1.TrayFreightService(company.id);
    let cursor = null;
    let scanned = 0;
    let queued = 0;
    let updated = 0;
    ...
    while (true) {
      ...
      `, [company.id, cursor || '', PAGE_SIZE]);
        ...
        const pendingOrders = orders.filter((order) => (0, freightRecalculationService_1.needsFreightRecalculation)(order));
        ...
        for (const order of pendingOrders) {
            try {
                ...
                if (!result.skipped) {
                    updated += 1;
                    console.log(`  OK ${order.orderNumber}: recalculado em R$ ${(result.quotedValue ?? 0).toFixed(2)}`);
                }
            }
            catch (error) {
                failed += 1;
    ...
    console.log(`Resumo ${company.name}: ${updated} atualizado(s), ${failed} com erro, ${Math.max(scanned - queued, 0)} ja estavam preenchidos no recorte.`);
    return {
        scanned,
        queued,
        updated,
