...
const parseJetCsv = (csvContent) => {
    ...
    const headers = parseCsvLine(headerLine, delimiter).map((header) => normalizeKey(header));
    const parsedRows = [];
...
const buildMapFromCsvRows = (rows) => {
    const mapping = new Map();
    for (const row of rows) {
        const idOrder = normalizeDigits(row.idorder) ||
...
async function main() {
    ...
    let csvMap = new Map();
    if (MAP_CSV_PATH) {
        const absoluteCsvPath = path_1.default.resolve(MAP_CSV_PATH);
        if (!fs_1.default.existsSync(absoluteCsvPath)) {
            throw new Error(`Arquivo CSV nao encontrado: ${absoluteCsvPath}`);
        }
        const csvContent = fs_1.default.readFileSync(absoluteCsvPath, 'utf8');
    ...
    const mergedMap = new Map(csvMap);
