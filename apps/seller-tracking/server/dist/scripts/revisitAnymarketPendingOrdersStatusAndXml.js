...
const INCLUDE_ARCHIVED = ['1', 'true', 'yes', 'sim'].includes(String(process.env.INCLUDE_ARCHIVED || '').trim().toLowerCase());
const IGNORE_CARRIER_EXCEPTIONS = ['1', 'true', 'yes', 'sim'].includes(String(process.env.IGNORE_CARRIER_EXCEPTIONS || 'true').trim().toLowerCase());
...
async function main() {
    ...
    console.log(`Incluir arquivados: ${INCLUDE_ARCHIVED ? 'SIM' : 'NAO'} | Ignorar excecoes de transportadora: ${IGNORE_CARRIER_EXCEPTIONS ? 'SIM' : 'NAO'}`);
    console.log(`Limit/offset por empresa: ${LIMIT_PER_COMPANY > 0 ? LIMIT_PER_COMPANY : 'sem limite'} / ${OFFSET_PER_COMPANY > 0 ? OFFSET_PER_COMPANY : 0}`);
    let totalOrders = 0;
    ...
    for (const company of companies) {
        const api = new anymarketApiService_1.AnymarketApiService(company.id);
        const targetOrders = await listTargetOrders(company.id);
        console.log(`\nEmpresa: ${company.name} (${company.id})`);
        const companyPendingCount = targetOrders.filter((row) => row.revisitReason === 'PENDING_STATUS').length;
        const companyMissingXmlActiveCount = targetOrders.filter((row) => row.revisitReason === 'MISSING_XML_ACTIVE').length;
        console.log(`Pedidos alvo no banco: ${targetOrders.length} (PENDENTE=${companyPendingCount}, sem XML ativo=${companyMissingXmlActiveCount})`);
        ...
        for (let index = 0; index < targetOrders.length; index += 1) {
            ...
            const reasonLabel = order.revisitReason === 'MISSING_XML_ACTIVE'
                ? 'SEM_XML_ATIVO'
            ...
            try {
                ...
                if (!anymarketOrder) {
                    companyNotFound += 1;
                    console.log(`  [${progress}] ${orderNumber} [${reasonLabel}]: nao encontrado na AnyMarket`);
                    continue;
                }
                companyFoundOnAny += 1;
                ...
                if (!APPLY_CHANGES) {
                    console.log(`  [${progress}] ${orderNumber} [${reasonLabel}]: status atual any=${normalizeText(anymarketOrder?.status) || '-'} | chaveNF=${mapped.invoiceAccessKey ? 'SIM' : 'NAO'}`);
                    continue;
                }
                ...
                const importedNow = importResult.results.created +
                ...
                console.log(`  [${progress}] ${orderNumber} [${reasonLabel}]: criado=${importResult.results.created} atualizado=${importResult.results.updated} ignorado=${importResult.results.skipped}`);
            }
            catch (error) {
                companyErrors += 1;
                console.log(`  [${progress}] ${orderNumber} [${reasonLabel}]: erro=${formatError(error)}`);
            }
        }
        ...
        console.log(`Resumo ${company.name}: encontradosAny=${companyFoundOnAny}, importados=${companyImported}, criados=${companyCreated}, atualizados=${companyUpdated}, ignorados=${companySkipped}, naoEncontrados=${companyNotFound}, xmlPreenchido=${companyXmlFilled}, erros=${companyErrors}`);
    }
    console.log('\n===== Resumo Geral =====');
...
main().catch((error) => {
    console.error('Falha no script de revisita AnyMarket para pedidos PENDENTE:', formatError(error));
    process.exitCode = 1;
});
