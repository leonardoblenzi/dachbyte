...
const listOrdersWithoutXml = async (companyId) => {
    const result = await (0, db_1.dbQuery)(`
      SELECT
        o."id",
        o."orderNumber",
...
const fetchOrderByCompanyAndNumber = async (companyId, orderNumber) => {
    const result = await (0, db_1.dbQuery)(`
      SELECT
        o."id",
        o."orderNumber",
...
async function main() {
    ...
    for (let index = 0; index < targetOrders.length; index += 1) {
        ...
        try {
            ...
            if (!APPLY_CHANGES) {
                console.log(`  [${progress}] ${orderNumber}: statusAny=${normalizeText(anyOrder?.status) || '-'} chaveNF=${hasXmlNow ? 'SIM' : 'NAO'}`);
                continue;
            }
            ...
            if (syncResult.success) {
                ...
                syncFailed += 1;
                console.log(`  [${progress}] ${orderNumber}: sync Intelipost falhou (${syncResult.message})`);
            }
        }
        catch (error) {
            errors += 1;
...
main().catch((error) => {
    console.error('Falha no script de revisita MPOZENATO sem XML + sync Intelipost:', formatError(error));
    process.exitCode = 1;
});
