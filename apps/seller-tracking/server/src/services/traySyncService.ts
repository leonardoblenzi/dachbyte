...
import {
  refreshInvoiceXmlRevisitState,
} from './integrationInvoiceXmlRevisitService';
...
export interface TraySyncFiltersInput {
  ...
  statuses?: string[];
  revisitShippingOverdue?: boolean;
}
...
export class TraySyncService {
  async executeSync(
    ...
    const storedOrders: any[] = storedOrdersResult.rows;
    ...
    const removedOrderIds: Set<string> = new Set(
      ordersToRemoveByCarrierException.map((order) => String(order.id)),
    );
    const eligibleStoredOrders = storedOrders.filter(
      (order) => !removedOrderIds.has(order.id),
    );
    const existingOrderNumbers: Set<string> = new Set(
    ...
    hooks?.onLog?.(
      `Sincronizacao Tray iniciada com janela de alteracoes dos ultimos ${days} dias e ${statusesToSync.length} status.`,
    );
    if (pendingInvoiceXmlOrderNumbers.size > 0) {
      hooks?.onLog?.(
        `${pendingInvoiceXmlOrderNumbers.size} pedido(s) existente(s) sem NF/Chave NF serao revisitados na Tray.`,
      );
    }
        ...
        },
        {
          ...
          onOrdersBatch: async (batchOrders) => {
            ...
            const revisitedOrders = ordersToProcess.filter((order) =>
              pendingInvoiceXmlOrderNumbers.has(String(order.id)),
            );
            ...
            if (affectedOrderIds.length > 0) {
              const ordersForFreightResult = await dbQuery<any>(
                `
                  SELECT
                    o."id",
                    o."orderNumber",
            ...
            for (const orderNumber of processedOrderNumbers) {
              ...
              const shouldSkipOrder =
                !pendingInvoiceXmlOrderNumbers.has(orderNumber);
            ...
            hooks?.onLog?.(
              `Lote importado no banco: ${importResult.results.created} criado(s), ${importResult.results.updated} atualizado(s), ${revisitedOrders.length} revisitado(s) por falta de NF/Chave NF.`,
            );
          },
        },
      );
    ...
    const remainingOrderNumbers: string[] = Array.from(
      pendingInvoiceXmlOrderNumbers,
    );
    ...
    if (remainingOrderNumbers.length > 0) {
      ...
      hooks?.onLog?.(
        `Revisita direta iniciada para ${remainingOrderNumbers.length} pedido(s) fora da janela automatica (${pendingIdentifierCount} sem NF/Chave NF).`,
      );
    ...
    if (filters.revisitShippingOverdue === true) {
      const shippingOverdueResult = await dbQuery<{ orderNumber: string }>(
        `
      ...
      const shippingOverdueOrderNumbers = shippingOverdueResult.rows.map((order) =>
        String(order.orderNumber),
      );

      hooks?.onLog?.(
        `Verificacao Tray iniciada para ${shippingOverdueOrderNumbers.length} pedido(s) com envio atrasado.`,
      );

      for (
        let batchStart = 0;
        batchStart < shippingOverdueOrderNumbers.length;
        batchStart += DIRECT_REVISIT_BATCH_SIZE
      ) {
        if (hooks?.shouldCancel?.()) {
          throw new SyncCancellationError();
        ...
        const batchOrderNumbers = shippingOverdueOrderNumbers.slice(
          batchStart,
          batchStart + DIRECT_REVISIT_BATCH_SIZE,
        );
        const checkedOrders = (
            ...
            batchOrderNumbers.map(async (orderNumber) => {
              try {
                ...
                const message =
                  error instanceof Error ? error.message : 'erro desconhecido';
                aggregateResults.errors.push(
                  `Pedido ${orderNumber}: falha ao verificar envio atrasado na Tray (${message})`,
                );
                hooks?.onLog?.(
                  `Falha ao verificar o pedido atrasado ${orderNumber} na Tray: ${message}`,
                );
                return null;
              }
            }),
          ...
          (entry): entry is { orderNumber: string; order: any } => Boolean(entry),
        );
        ...
        const shippedOrders = checkedOrders
          ...
          .filter(
            (order) =>
              order && String(order.status || '').trim().toUpperCase() === 'ENVIADO',
          );
        ...
        const mappedOrders = shippedOrders.map((order) =>
          trayApi.mapTrayOrderToSystem(order, {
            companyName: company?.name,
          }),
        );
        ...
        hooks?.onLog?.(
          `${shippedOrders.length} pedido(s) atrasado(s) retornaram ENVIADO na Tray; ${importResult.results.updated} atualizado(s) na plataforma.`,
        );
      }

      hooks?.onLog?.(
        `Verificacao de envio atrasado concluida: ${shippingOverdueCheckedCount} consultado(s), ${shippingOverdueUpdatedCount} atualizado(s) para enviado.`,
      );
    }

    if (processedOrdersCount === 0) {
      const emptyMessage =
        filters.revisitShippingOverdue === true
