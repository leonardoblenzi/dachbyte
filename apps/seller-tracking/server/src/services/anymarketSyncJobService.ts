...
import { normalizeAnymarketStatusValue } from './integrationOrderStatusService';
...
const ANYMARKET_AUTO_SYNC_STATUS_SET = new Set<string>(
...
const normalizeConfiguredStatuses = (value: unknown) => {
  ...
  return Array.from(
    new Set(
      value
        .map((item) => normalizeAnymarketStatusValue(item))
...
class AnymarketSyncJobService {
  ...
  ensureSchedule(companyId: string, userId: string) {
    ...
    if (existing) {
      ...
      if (!existing.nextScheduledAt) this.scheduleNext(companyId, userId);
      return;
    }
  ...
  async initializeSchedules() {
    ...
    for (const company of companiesResult.rows) {
      if (isDemoCompany(company)) continue;
  ...
  private triggerAutomaticSync(companyId: string) {
    ...
    void (async () => {
      try {
        ...
        if (
          company?.anymarketIntegrationEnabled !== true ||
          !tokenConfigured ||
        ...
        const configuredStatuses = normalizeConfiguredStatuses(
        ...
        this.pushLog(
          job,
          'info',
          `Execucao automatica ANYMARKET disparada com pedidos novos e alterados dos ultimos 2 dias em ${statuses.length} status valido(s).`,
        );
      } catch (error) {
