...
function normalizeEnforcement() {
  const raw = String(process.env.HUB_AUTH_MODE || process.env.HUB_ENFORCEMENT || "strict")
    .trim()
    .toLowerCase();
...
function buildIdentity(input) {
  ...
  const workspaceId = String(workspace?.id || user.workspaceId || "").trim();
  if (!workspaceId) {
    if (user.isMaster) {
      return { ok: true, bypass: true, reason: "master_bypass_without_workspace" };
    }
    return { ok: false, reason: "identity_workspace_missing" };
  }

  const tenantGlobalId = String(workspace?.tenantGlobalId || "").trim();
  const tenantId = tenantGlobalId || `madeira_workspace_${workspaceId}`;
  const companyName = String(workspace?.name || workspace?.sellerName || "").trim();
  const userId = `madeira_user_${user.id}`;
  ...
  return {
    ...
    payload: {
      ...
      document_type: normalizeDocumentType(workspace?.documentType),
      document_number: normalizeDocumentNumber(workspace?.documentNumber),
      user_id: userId,
      full_name: fullName,
      email,
...
async function evaluateHubLoginAccess(input) {
  ...
  try {
    ...
    return {
      ...
      expires_at: data.expires_at || data.ends_at || null,
