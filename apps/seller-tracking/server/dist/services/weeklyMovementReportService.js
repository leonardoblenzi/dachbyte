...
exports.weeklyMovementReportService = void 0;
...
const getReportsDir = () => path_1.default.join(__dirname, '../../public/reports/weekly-summary');
...
class WeeklyMovementReportService {
    ...
    buildStatusSummary(orders) {
        const summary = Object.values(orderStatus_1.OrderStatus).map((status) => ({
            status,
    ...
    buildReportHtml(input) {
        const bars = buildMetricBars(input.statusSummary.map((item, index) => ({
            label: item.label,
            value: item.count,
            color: ['#64748b', '#2563eb', '#0ea5e9', '#7c3aed', '#10b981', '#ef4444', '#f59e0b', '#334155', '#8b5cf6'][index % 9],
        })));
        const movementRows = input.movedOrders.length > 0
            ? input.movedOrders
                .map((order) => `
                <tr>
          ...
          <section style="display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,0.9fr);gap:20px;margin-top:26px;align-items:start;">
            ...
            <div style="border:1px solid #dbeafe;border-radius:22px;padding:22px;background:#f8fbff;">
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;font-size:14px;">
                <tbody>
                  ${input.statusSummary
            .map((item) => `<tr><td style="padding:8px 0;border-top:1px solid #dbeafe;">${escapeHtml(item.label)}</td><td align="right" style="padding:8px 0;border-top:1px solid #dbeafe;">${item.count}</td></tr>`)
            .join('')}
                </tbody>
              </table>
            </div>
...
<html lang="pt-BR">
  ...
  <body style="margin:0;padding:0;background:#0b1220;font-family:Arial,Helvetica,sans-serif;color:#0f172a;">
    ...
    async sendCompanyReport(companyId) {
      ...
      `, [company.id]);
        ...
        const movedOrders = orders.filter((order) => order.createdAt >= periodStart ||
        ...
        `);
            ...
            for (const company of companies) {
                try {
                    ...
                    console.error(`Erro ao enviar relatorio semanal da empresa ${company.id}:`, error);
                }
            }
        }
        catch (error) {
...
exports.weeklyMovementReportService = new WeeklyMovementReportService();
