...
class SyncJobService {
  ...
  private autoSyncUsersByCompany: Map<string, string> = new Map();
  private autoSchedule: AutoScheduleState = {
    nextScheduledAt: null,
    timeout: null,
  };
  private autoRunInProgress = false;
  ...
  private triggerAutomaticBatch() {
    ...
    void this.runAutomaticBatch()
      .catch((error) => {
        console.error(
          'Erro ao executar lote automatico de sincronizacao de rastreio:',
          error,
        );
      })
      .finally(() => {
  ...
  private async runAutomaticBatch() {
    if (!shouldRunAutomaticProcesses()) {
      return;
    }
