...
const listPendingOrders = async (companyId) => {
    const result = await (0, db_1.dbQuery)(`
      SELECT
        o."id",
        o."orderNumber",
...
async function main() {
    ...
    if (company.anymarketIntegrationEnabled === false || !normalizeText(company.anymarketToken)) {
        console.log(`Empresa ${company.name} sem integracao AnyMarket ativa/token salvo. Operacao cancelada.`);
        return;
    }
    if (company.intelipostIntegrationEnabled === false ||
        !normalizeText(company.intelipostApiKey)) {
        console.log(`Empresa ${company.name} sem Intelipost ativa/api-key salva. Operacao cancelada.`);
        return;
    }
    ...
    console.log(`Arquivados: NAO | Ignorar excecoes transportadora: ${IGNORE_CARRIER_EXCEPTIONS ? 'SIM' : 'NAO'}`);
    console.log(`Limite/offset: ${LIMIT > 0 ? LIMIT : 'sem limite'} / ${OFFSET > 0 ? OFFSET : 0}`);
    ...
    for (let index = 0; index < orders.length; index += 1) {
        ...
        try {
            ...
            if (!APPLY_CHANGES) {
                console.log(`  [${progress}] ${orderNumber}: statusAny=${normalizeText(anymarketOrder?.status) || '-'} chaveNF=${hasXmlAfterLookup ? 'SIM' : 'NAO'}`);
            ...
            if (!hasXmlAfterLookup) {
                skippedNoXml += 1;
                console.log(`  [${progress}] ${orderNumber}: importado sem chave XML, sync Intelipost por XML ignorado.`);
                continue;
            }
            readyForIntelipostByXml += 1;
            syncRequested += 1;
            ...
            if (syncResult.success) {
                syncSuccess += 1;
                console.log(`  [${progress}] ${orderNumber}: importado + sync Intelipost XML OK.`);
            }
            else {
                console.log(`  [${progress}] ${orderNumber}: importado + sync Intelipost XML sem sucesso (${syncResult.message}).`);
            }
        }
        catch (error) {
            errors += 1;
...
main().catch((error) => {
    console.error('Falha no script MPOZENATO pendentes + AnyMarket XML + Intelipost XML:', formatError(error));
    process.exitCode = 1;
});
