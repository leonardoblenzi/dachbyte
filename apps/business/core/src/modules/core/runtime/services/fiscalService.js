...
const { recordEvent } = require("./persistenceHelpers");
const { assertCompanyCapability } = require("./configurationService");
...
async function listFiscalDocuments(companyId) {
  const result = await db.query(`select f.id, f.sale_id as "saleId", f.customer_id as "customerId", c.name as "customerName",
      f.model, f.status, f.metadata, f.created_at as "createdAt", f.updated_at as "updatedAt"
    from volt_core.fiscal_documents f left join volt_core.customers c on c.id = f.customer_id and c.company_id = f.company_id
    where f.company_id = $1 order by f.created_at desc;`, [companyId]);
  return result.rows;
}
