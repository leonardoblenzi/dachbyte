...
exports.resolveOrderShippingAvailabilityFromProductPayloads = exports.resolveOrderShippingAvailabilityFromTray = exports.resolveOrderShippingAvailabilityFromAnymarket = exports.extractTrayProductReferences = exports.extractAnymarketProductReferences = void 0;
...
const readPathValue = (payload, path) => {
...
const pickFirstPositive = (payload, paths) => {
...
const inferModeFromPayload = (payload, fallback) => {
...
const toMonthDayKey = (value) => `${String(value.getMonth() + 1).padStart(2, '0')}-${String(value.getDate()).padStart(2, '0')}`;
...
const buildHolidaySetForYear = (year) => {
    const keys = new Set();
...
const addCalendarDays = (baseDate, calendarDays) => {
    const next = new Date(baseDate);
    next.setHours(12, 0, 0, 0);
    next.setDate(next.getDate() + calendarDays);
    return next;
};
const buildResolution = (items, baseDate) => {
    if (items.length === 0) {
        return {
            maxDays: null,
            mode: 'UNKNOWN',
    ...
    const modePriority = items.some((item) => item.mode === 'BUSINESS')
        ? 'BUSINESS'
        : items.some((item) => item.mode === 'CALENDAR')
            ? 'CALENDAR'
            : 'UNKNOWN';
    const resolvedMode = modePriority === 'UNKNOWN' ? 'CALENDAR' : modePriority;
    const sendDate = resolvedMode === 'BUSINESS'
...
const resolveAnymarketAdditionalTimeFromProductPayload = (payload, productRef) => {
...
const resolveAnymarketItems = (orderPayload) => {
    ...
    return items.map((item, index) => {
        ...
        const productRef = String(skuId || skuPartnerId || productId || `item_${index + 1}`);
...
const resolveTrayItems = (orderPayload) => {
    const productNodes = [];
    ...
    return productNodes.map((item, index) => {
        const productRef = String(item?.product_id || item?.id || item?.code || `item_${index + 1}`);
        const days = pickFirstPositive(item, [
            'availability_days',
            'availabilityDays',
            'release_days',
...
const extractAnymarketProductReferences = (orderPayload) => {
    const items = Array.isArray(orderPayload?.items) ? orderPayload.items : [];
    const refs = [];
...
exports.extractAnymarketProductReferences = extractAnymarketProductReferences;
const extractTrayProductReferences = (orderPayload) => {
    const refs = [];
    const productsSold = Array.isArray(orderPayload?.ProductsSold)
        ? orderPayload.ProductsSold
        : [];
...
exports.extractTrayProductReferences = extractTrayProductReferences;
const resolveOrderShippingAvailabilityFromAnymarket = (orderPayload, baseDate) => buildResolution(resolveAnymarketItems(orderPayload).filter(Boolean), baseDate);
exports.resolveOrderShippingAvailabilityFromAnymarket = resolveOrderShippingAvailabilityFromAnymarket;
const resolveOrderShippingAvailabilityFromTray = (orderPayload, baseDate) => buildResolution(resolveTrayItems(orderPayload).filter(Boolean), baseDate);
exports.resolveOrderShippingAvailabilityFromTray = resolveOrderShippingAvailabilityFromTray;
const resolveOrderShippingAvailabilityFromProductPayloads = (inputs, baseDate) => {
    const snapshots = [];
    for (const input of inputs) {
        const payload = input.payload;
        const days = input.integration === 'ANYMARKET'
        ...
        const mode = inferModeFromPayload(payload, input.integration === 'TRAY' ? 'BUSINESS' : 'CALENDAR');
...
exports.resolveOrderShippingAvailabilityFromProductPayloads = resolveOrderShippingAvailabilityFromProductPayloads;
