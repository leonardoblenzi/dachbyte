...
async function createOpticalOrderWithClient(client, companyId, sale, customer, items, optical = {}, actorUserId) {
  ...
  const frame = items.find((item) => item.product.id === optical.frameProductId || item.product.opticalType === "frame");
  const lens = items.find((item) => item.product.id === optical.lensProductId || item.product.opticalType === "lens");
