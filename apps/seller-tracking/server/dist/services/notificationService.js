...
const MONITORED_WATCH_EVENT_LABELS = {
...
const escapeHtml = (value) => String(value ?? '')
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const normalizeWatchEvents = (value) => {
    const normalized = Array.isArray(value)
        ? value
            .map((item) => String(item || '').trim().toUpperCase())
            .filter((item) => MONITORED_WATCH_EVENT_SET.has(item))
        : [];
...
class NotificationService {
    async getFeed(companyId, limit = 40) {
        let generalRows = [];
        let monitoredRows = [];
        let monitoredOrderRows = [];
        try {
          ...
          `, [companyId]),
            ...
            console.warn('[notifications] Fallback de feed ativado por incompatibilidade de schema:', error);
            return {
                general: [],
                monitored: [],
                monitoredOrderIds: [],
        ...
        const activeMonitoredOrderRows = normalizedMonitoredOrderRows
            ...
            return {
                ...
                statusLabel: STATUS_LABELS[order.status] || String(order.status || ''),
                lastUpdate: safeDateString(order.lastUpdate),
                createdAt: safeDateString(item.createdAt),
            };
        })
            .filter(Boolean);
        let monitoredOrderFallbackRows = [];
        if (activeMonitoredOrderRows.length === 0) {
            const monitoredHistoryResult = await (0, db_1.dbQuery)(`
          SELECT
            sn."id",
            sn."payload",
            sn."createdAt"
        ...
        `, [companyId, Math.max(limit * 5, 100)]);
            ...
            monitoredOrderFallbackRows = (Array.isArray(monitoredHistoryRows)
                ...
                const parsedStatus = Object.values(orderStatus_1.OrderStatus).includes(status)
                    ? status
                    : null;
                return {
        ...
        const monitoredOrdersMap = new Map();
        ...
        return {
            ...
            unreadCount: (Array.isArray(generalRows) ? generalRows.length : 0) +
                (Array.isArray(monitoredRows) ? monitoredRows.length : 0),
        };
    }
    async markAllAsRead(companyId) {
        const updated = await (0, db_1.dbQuery)(`
    ...
    async addMonitoredOrders(input) {
        const hasExplicitWatchEvents = Array.isArray(input.watchEvents) && input.watchEvents.length > 0;
        ...
        const normalizedIdentifiers = Array.from(new Set((Array.isArray(input.identifiers) ? input.identifiers : [])
            ...
            .filter(Boolean)));
        const normalizedOrderIds = Array.from(new Set((Array.isArray(input.orderIds) ? input.orderIds : [])
            .map((item) => normalizeIdentifier(item))
            .filter(Boolean)));
        if (normalizedOrderIds.length > 0) {
            const ordersByIdResult = await (0, db_1.dbQuery)(`
          SELECT o."id"
          FROM "Order" o
          WHERE o."companyId" = $1
            AND o."isArchived" = FALSE
        ...
        for (const identifier of normalizedIdentifiers) {
            ...
            const matchResult = await (0, db_1.dbQuery)(`
          SELECT o."id"
          FROM "Order" o
          WHERE o."companyId" = $1
            AND o."isArchived" = FALSE
        ...
        if (allCandidateOrderIds.length === 0) {
            ...
            return {
                addedOrders: [],
                alreadyMonitoredOrders: [],
                notFoundIdentifiers,
                monitoredOrderIds: monitoredIds,
            };
        }
        const orderDetailsResult = await (0, db_1.dbQuery)(`
        SELECT
          o."id",
          o."orderNumber",
          o."invoiceNumber"
      ...
      `, [input.companyId, allCandidateOrderIds]);
        ...
        const detailsById = new Map((Array.isArray(orderDetails) ? orderDetails : []).map((item) => [
            String(item.id),
            {
                id: String(item.id),
        ...
        const existingRowsResult = await (0, db_1.dbQuery)(`
        SELECT
          mo."id",
          mo."orderId"
        FROM "MonitoredOrder" mo
      ...
      `, [input.companyId, allCandidateOrderIds]);
        ...
        const existingOrderIdSet = new Set((Array.isArray(existingRows) ? existingRows : [])
            .map((item) => String(item.orderId || ''))
            .filter(Boolean));
        ...
        if (hasExplicitWatchEvents && existingRows.length > 0) {
            await Promise.all(existingRows.map((row) => (0, db_1.dbQuery)(`
            UPDATE "MonitoredOrder"
            SET "watchEvents" = $2::text[]
            WHERE "id" = $1
          `, [row.id, normalizedWatchEvents])));
        }
        const currentMonitoredCountResult = await (0, db_1.dbQuery)(`
        SELECT COUNT(*)::int AS "count"
        FROM "MonitoredOrder"
        WHERE "companyId" = $1
      `, [input.companyId]);
        const currentMonitoredCount = Number(currentMonitoredCountResult.rows[0]?.count || 0);
        const availableSlots = Math.max(0, MAX_MONITORED_ORDERS_PER_COMPANY - Number(currentMonitoredCount || 0));
        ...
        return {
            ...
            availableSlotsAfter: MAX_MONITORED_ORDERS_PER_COMPANY - monitoredOrderIds.length,
            notFoundIdentifiers,
            monitoredOrderIds,
        };
    }
    async removeMonitoredOrder(companyId, orderId) {
    ...
    async getMonitoredOrderIds(companyId) {
    ...
    async sendMonitoredEnrollmentEmail(input) {
        const normalizedOrderIds = Array.from(new Set((Array.isArray(input.orderIds) ? input.orderIds : [])
            ...
            .filter(Boolean)));
        if (normalizedOrderIds.length === 0) {
            return { sent: false, reason: 'NO_ORDERS' };
        }
        const [company, recipients, orders] = await Promise.all([
          ...
          WHERE c."id" = $1
          LIMIT 1
        `, [input.companyId]).then((result) => result.rows[0] || null),
          ...
          WHERE o."companyId" = $1
            AND o."id" = ANY($2::text[])
          ORDER BY o."lastUpdate" DESC
        `, [input.companyId, normalizedOrderIds]).then((result) => result.rows),
        ]);
        if (!recipients.length) {
            return { sent: false, reason: 'NO_RECIPIENTS' };
        }
        if (!orders.length) {
            return { sent: false, reason: 'NO_MATCHED_ORDERS' };
        }
    ...
    async registerTrackingSyncNotifications(input) {
        ...
        const summaryMessage = `Entregues: ${deliveredCount} | Entraram em atraso: ${enteredDelayCount} | ` +
    ...
    async registerIntegrationHealthNotification(input) {
        await (0, db_1.dbQuery)(`
        INSERT INTO "SyncNotification" (
          "id",
          "companyId",
      ...
      `, [
        ...
        `,
            ...
            console.error(`Erro ao enviar e-mail de saude da integracao ${input.integration} para a empresa ${input.companyId}:`, error);
        }
        return { sent: true, recipientCount: recipients.length };
    }
    async registerMonitoredOrderChanges(companyId, changes) {
        await this.createMonitoredOrderNotifications(companyId, changes);
    }
    async releaseClosedMonitoredOrders(companyId) {
        await (0, db_1.dbQuery)(`
    ...
    async resolveCompanyRecipients(companyId) {
    ...
    buildMonitoredEventEmail(input) {
    ...
    async createMonitoredOrderNotifications(companyId, changes) {
        ...
        const monitoredRowsResult = await (0, db_1.dbQuery)(`
        SELECT
          mo."orderId",
          mo."watchEvents",
          o."invoiceNumber"
      ...
      `, [companyId]);
        ...
        const monitoredOrderIdsToRelease = new Set();
        ...
        for (const change of candidateChanges) {
            ...
            const shouldNotify = (event) => watchEvents.has('ALL') || watchEvents.has(event);
            ...
            if (change.changed &&
                ...
                detectedEvents.push(`Status alterado de ${previousStatusLabel} para ${currentStatusLabel}.`);
            ...
            if (change.previousStatus !== change.currentStatus &&
                change.currentStatus === orderStatus_1.OrderStatus.DELIVERED &&
            ...
            if (change.previousStatus !== change.currentStatus &&
                change.currentStatus === orderStatus_1.OrderStatus.CANCELED &&
            ...
            if (change.previousStatus !== change.currentStatus &&
                change.currentStatus === orderStatus_1.OrderStatus.SHIPPED &&
            ...
            if ((change.enteredRoute ||
                (change.previousStatus !== change.currentStatus &&
                    change.currentStatus === orderStatus_1.OrderStatus.DELIVERY_ATTEMPT)) &&
                shouldNotify('ROUTE')) {
            ...
            if (detectedEvents.length > 0 && recipients.length > 0) {
                ...
                try {
                    ...
                    console.error(`Falha ao enviar e-mail do pedido monitorado ${orderLabel}:`, emailError);
                }
            }
        }
