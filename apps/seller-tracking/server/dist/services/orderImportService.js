...
exports.importOrdersForCompany = void 0;
...
const mapStatus = (status) => {
    const statusMap = {
        ...
        CREATED: orderStatus_1.OrderStatus.CREATED,
        SHIPPED: orderStatus_1.OrderStatus.SHIPPED,
        DELIVERY_ATTEMPT: orderStatus_1.OrderStatus.DELIVERY_ATTEMPT,
        DELIVERED: orderStatus_1.OrderStatus.DELIVERED,
        FAILURE: orderStatus_1.OrderStatus.FAILURE,
        RETURNED: orderStatus_1.OrderStatus.RETURNED,
        CANCELED: orderStatus_1.OrderStatus.CANCELED,
        CHANNEL_LOGISTICS: orderStatus_1.OrderStatus.CHANNEL_LOGISTICS,
    };
    return statusMap[status] || orderStatus_1.OrderStatus.PENDING;
};
const STATUS_LABELS = {
    PENDING: 'Pendente',
    CREATED: 'Criado',
...
const parseShippingCutoffTime = (value) => {
...
const parseDateAndHour = (dateValue, hourValue) => {
...
const resolveShippingAvailabilityMaxDays = (orderData) => {
    const rawValue = orderData?.apiRawPayload?.shippingAvailability?.maxDays ??
        orderData?.apiRawPayload?.anymarketMeta?.shippingAvailability?.maxDays ??
        orderData?.shippingAvailability?.maxDays;
    const parsed = Number(rawValue);
    return Number.isFinite(parsed) ? parsed : null;
};
const applyCutoffRuleToZeroDayDeadline = (orderData, maxShippingDeadline, shippingCutoffTime) => {
    if (!maxShippingDeadline)
    ...
    const createdMinutes = platformCreatedAt.getHours() * 60 + platformCreatedAt.getMinutes();
...
const isActiveDelayedByCarrier = (status, carrierEstimatedDeliveryDate) => {
    ...
    return Boolean(carrierEstimatedDeliveryDate &&
        !closedStatuses.includes(status) &&
        new Date() > carrierEstimatedDeliveryDate);
};
const buildTrackingEventsData = (orderData, fallbackStatus) => {
...
const normalizeDate = (value) => value ? value.getTime() : null;
const formatValueDate = (value) => value ? value.toLocaleDateString('pt-BR') : '-';
const formatValueCurrency = (value) => new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL',
}).format(Number(value || 0));
const describeUpdatedFields = (existing, nextOrder) => {
    const updatedFields = [];
    if (existing.status !== nextOrder.status) {
        updatedFields.push(`Status (${STATUS_LABELS[existing.status]} -> ${STATUS_LABELS[nextOrder.status]})`);
    }
    if (normalizeText(existing.invoiceNumber) !== normalizeText(nextOrder.invoiceNumber)) {
        updatedFields.push(`NF (${normalizeText(existing.invoiceNumber) || '-'} -> ${normalizeText(nextOrder.invoiceNumber) || '-'})`);
    }
    if (normalizeText(existing.invoiceAccessKey) !==
        normalizeText(nextOrder.invoiceAccessKey)) {
        updatedFields.push(`Chave NF (${normalizeText(existing.invoiceAccessKey) || '-'} -> ${normalizeText(nextOrder.invoiceAccessKey) || '-'})`);
    }
    if (normalizeText(existing.trackingCode) !== normalizeText(nextOrder.trackingCode)) {
        updatedFields.push(`Codigo de rastreio (${normalizeText(existing.trackingCode) || '-'} -> ${normalizeText(nextOrder.trackingCode) || '-'})`);
    }
    if (normalizeText(existing.customerName) !== normalizeText(nextOrder.customerName)) {
        updatedFields.push(`Cliente (${normalizeText(existing.customerName)} -> ${normalizeText(nextOrder.customerName)})`);
    }
    if (normalizeText(existing.salesChannel) !== normalizeText(nextOrder.salesChannel)) {
        updatedFields.push(`Canal (${normalizeText(existing.salesChannel)} -> ${normalizeText(nextOrder.salesChannel)})`);
    }
    if (normalizeText(existing.freightType) !== normalizeText(nextOrder.freightType)) {
        updatedFields.push(`Frete (${normalizeText(existing.freightType) || '-'} -> ${normalizeText(nextOrder.freightType) || '-'})`);
    }
    if (normalizeDate(existing.shippingDate) !== normalizeDate(nextOrder.shippingDate)) {
        updatedFields.push(`Data de envio (${formatValueDate(existing.shippingDate)} -> ${formatValueDate(nextOrder.shippingDate)})`);
    }
    if (normalizeDate(existing.estimatedDeliveryDate) !==
        normalizeDate(nextOrder.estimatedDeliveryDate)) {
        updatedFields.push(`Previsao de entrega (${formatValueDate(existing.estimatedDeliveryDate)} -> ${formatValueDate(nextOrder.estimatedDeliveryDate)})`);
    }
    if (normalizeDate(existing.carrierEstimatedDeliveryDate) !==
        normalizeDate(nextOrder.carrierEstimatedDeliveryDate)) {
        updatedFields.push(`Previsao da transportadora (${formatValueDate(existing.carrierEstimatedDeliveryDate)} -> ${formatValueDate(nextOrder.carrierEstimatedDeliveryDate)})`);
    }
    if (Number(existing.totalValue) !== Number(nextOrder.totalValue)) {
        updatedFields.push(`Valor (${formatValueCurrency(Number(existing.totalValue || 0))} -> ${formatValueCurrency(nextOrder.totalValue)})`);
    }
    if (existing.isDelayed !== nextOrder.isDelayed) {
        updatedFields.push(`Atraso (${existing.isDelayed ? 'Sim' : 'Nao'} -> ${nextOrder.isDelayed ? 'Sim' : 'Nao'})`);
    }
...
const buildOrderData = (orderData, status, options) => {
    const carrierEstimatedDeliveryDate = safeDate(orderData.carrierEstimatedDeliveryDate);
    const resolvedMaxShippingDeadline = applyCutoffRuleToZeroDayDeadline(orderData, safeDate(orderData.maxShippingDeadline), options?.shippingCutoffTime);
    return {
        ...
        quotedFreightValue: orderData.quotedFreightValue === null || orderData.quotedFreightValue === undefined
            ? null
        ...
        originalQuotedFreightValue: orderData.originalQuotedFreightValue === null ||
        ...
        originalQuotedFreightQuotationId: safeString(orderData.originalQuotedFreightQuotationId),
        recalculatedFreightValue: orderData.recalculatedFreightValue === null ||
...
const buildTraySyncOrderReport = (orderId, orderPayload, updateSummary) => ({
    orderId,
    ...
    carrierEstimatedDeliveryDate: orderPayload.carrierEstimatedDeliveryDate?.toISOString() || null,
    totalValue: orderPayload.totalValue,
    isDelayed: orderPayload.isDelayed,
    updateSummary,
});
const importOrdersForCompany = async (companyId, orders, options) => {
    if (!Array.isArray(orders) || orders.length === 0) {
        throw new Error('Nenhum pedido valido para importar');
    }
    const normalizedOrders = Array.from(new Map(orders.map((order, index) => [
    ...
    const skippedOrders = [];
    const errors = [];
    const registerSkippedOrder = (orderNumber, reason) => {
        skipped += 1;
        skippedOrders.push({
    ...
    for (const orderData of normalizedOrders) {
        ...
        if (!options?.ignoreCarrierExceptions &&
            (0, orderExclusion_1.shouldSkipPlatformOrderImport)({
                freightType: orderData?.freightType,
                carrierExceptions: company?.integrationCarrierExceptions,
            })) {
            registerSkippedOrder(orderNumber, 'Pedido bloqueado por excecao de transportadora');
            continue;
        }
        try {
          ...
          `, [
              ...
              `, [
                ...
                updatedOrders.push(buildTraySyncOrderReport(existing.id, orderPayload, updateSummary));
                continue;
            }
            const createdOrder = await (0, db_1.withDbTransaction)(async (client) => {
          ...
          `, [
            ...
            errors.push(`Pedido ${orderNumber}: ${message}`);
        }
    }
    const message = `Importacao concluida: ${created} criados, ${updated} atualizados, ` +
...
exports.importOrdersForCompany = importOrdersForCompany;
