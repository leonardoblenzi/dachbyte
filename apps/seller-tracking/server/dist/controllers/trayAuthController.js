...
exports.checkAuthStatus = exports.handleAuthCallback = exports.showInstallPage = exports.startTrayAuthorization = void 0;
...
const getAppBaseUrl = (req) => {
    const configuredBaseUrl = String(process.env.APP_BASE_URL || process.env.FRONTEND_URL || '').trim();
    if (configuredBaseUrl) {
        return applyBasePathToConfiguredUrl(configuredBaseUrl, getConfiguredBasePath());
    }
    return `${getRequestOrigin(req)}${getRequestBasePath(req)}`;
};
const getTrayCallbackUrl = (req) => {
    return `${getAppBaseUrl(req)}/tray/callback/auth`;
};
const getIntegrationReturnUrl = (req, status) => {
...
const startTrayAuthorization = async (req, res) => {
    const { url } = req.query;
    if (!req.user) {
    ...
    try {
        const companyToken = trayAuthService_1.trayAuthService.signCompanyContext(req.user.companyId, req.user.id);
        const authUrl = trayAuthService_1.trayAuthService.getAuthorizationUrl(String(url), {
            companyToken,
...
exports.startTrayAuthorization = startTrayAuthorization;
/**
 * GET /api/tray/callback
 * Landing page opcional de instalacao
 */
const showInstallPage = (req, res) => {
    const { url, adm_user, store, company_token } = req.query;
    if (!url) {
        return res.status(400).json({ error: 'A URL da loja Tray e obrigatoria' });
    }
    const normalizedStoreUrl = trayAuthService_1.trayAuthService.normalizeStoreUrl(String(url));
    const authUrl = trayAuthService_1.trayAuthService.getAuthorizationUrl(normalizedStoreUrl, {
        companyToken: typeof company_token === 'string' && company_token.trim()
            ? company_token.trim()
            : undefined,
        callbackUrl: getTrayCallbackUrl(req),
        fallbackOrigin: getRequestOrigin(req),
...
exports.showInstallPage = showInstallPage;
/**
...
const handleAuthCallback = async (req, res) => {
    try {
        ...
        const companyToken = typeof company_token === 'string' && company_token.trim()
            ? company_token.trim()
            : '';
        const context = trayAuthService_1.trayAuthService.verifyCompanyContext(companyToken);
        if (!code || !api_address || !store || !context?.companyId) {
            return res.redirect(getIntegrationReturnUrl(req, 'error'));
        }
        const authData = await trayAuthService_1.trayAuthService.generateAccessToken(String(code), String(api_address));
        const expirationDate = authData.date_expiration_access_token || authData.date_expiration;
        const refreshTokenExpirationDate = authData.date_expiration_refresh_token;
        const resolvedStoreId = String(authData.store_id || store);
        const resolvedApiAddress = String(authData.api_host || api_address);
        await trayAuthService_1.trayAuthService.saveAuth(context.companyId, {
            storeId: resolvedStoreId,
            apiAddress: resolvedApiAddress,
            accessToken: authData.access_token,
            refreshToken: authData.refresh_token,
            expiresAt: trayAuthService_1.trayAuthService.parseExpirationDate(expirationDate),
            refreshTokenExpiresAt: refreshTokenExpirationDate
                ? trayAuthService_1.trayAuthService.parseExpirationDate(refreshTokenExpirationDate)
                : null,
            code: String(code),
...
const checkAuthStatus = async (req, res) => {
    try {
        ...
        const auth = await trayAuthService_1.trayAuthService.getCurrentAuth(req.user.companyId, storeId);
        if (!auth) {
            return res.json({
                authorized: false,
                status: 'offline',
        ...
        try {
            isAuthorized = Boolean(await trayAuthService_1.trayAuthService.getValidAuth(req.user.companyId));
        }
        catch (error) {
