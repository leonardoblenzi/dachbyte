...
async function listHubResourceSyncCandidates({ limit = 100 } = {}) {
  const safeLimit = Math.min(100, Math.max(1, Number(limit) || 100));
  const { rows } = await db.query(
    `select id from magalu.accounts
      where hub_sync_status in ('pending', 'failed')
...
async function markHubResourceSyncPendingIfFailed(accountId) {
  const { rows } = await db.query(
    `update magalu.accounts
        set hub_sync_status = 'pending', hub_sync_error = null, updated_at = now()
      where id = $1 and hub_sync_status = 'failed'
      returning id`,
    [Number(accountId)],
  );
  return rows.length > 0;
