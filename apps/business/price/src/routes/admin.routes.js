...
function createIntegrationLinkHandler({
  ...
  return async (req, res, next) => {
    try {
      ...
      return res.status(201).json({ authorizationUrl: `${getBaseUrl(req)}/volt-price/api/integrations/link/${created.token}`, expiresAt: created.link.expires_at });
    } catch (error) { return next(error); }
  };
}
...
function createAdminRouter({
  ...
  router.post("/tenants/:id/support-session", requireCsrf, async (req, res, next) => { try { const reason = String(req.body?.reason || "").trim(); if (reason.length < 5) throw invalid("Informe o motivo do acesso assistido."); const created = await withClient(async (client) => { await client.query("BEGIN"); try { const tenant = (await client.query("SELECT id,name,status FROM volt_price.tenants WHERE id=$1", [req.params.id])).rows[0]; if (!tenant || tenant.status !== "active") throw Object.assign(new Error("Tenant indisponivel."), { statusCode: 409 }); await client.query("UPDATE volt_price.sessions SET revoked_at=now() WHERE id=$1", [req.vpAuth.sessionId]); const session = await auth.createSession(client, { userId: req.vpAuth.userId, tenantId: tenant.id, ip: req.ip, userAgent: req.get("user-agent"), supportReason: reason }); await audit(client, { tenantId: tenant.id, actorUserId: req.vpAuth.userId, actorType: "platform_admin", action: "support_session.start", resourceType: "tenant", resourceId: tenant.id, metadata: { reason }, ip: req.ip, userAgent: req.get("user-agent") }); await client.query("COMMIT"); return { session, tenant }; } catch (error) { await client.query("ROLLBACK"); throw error; } }); res.cookie(config.sessionCookie, created.session.token, auth.cookieOptions()); res.json({ success: true, csrfToken: created.session.csrf, tenant: created.tenant }); } catch (error) { next(error); } });
  router.post("/exit-support", requireCsrf, async (req, res, next) => { try { const created = await withClient(async (client) => { await client.query("BEGIN"); try { await client.query("UPDATE volt_price.sessions SET revoked_at=now() WHERE id=$1", [req.vpAuth.sessionId]); const session = await auth.createSession(client, { userId: req.vpAuth.userId, tenantId: null, ip: req.ip, userAgent: req.get("user-agent") }); await audit(client, { tenantId: req.vpAuth.tenantId, actorUserId: req.vpAuth.userId, actorType: "platform_admin", action: "support_session.end", resourceType: "tenant", resourceId: req.vpAuth.tenantId }); await client.query("COMMIT"); return session; } catch (error) { await client.query("ROLLBACK"); throw error; } }); res.cookie(config.sessionCookie, created.token, auth.cookieOptions()); res.json({ success: true, csrfToken: created.csrf }); } catch (error) { next(error); } });
  return router;
}
