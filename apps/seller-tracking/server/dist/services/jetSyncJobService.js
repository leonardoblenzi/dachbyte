...
exports.jetSyncJobService = void 0;
const crypto_1 = __importDefault(require("crypto"));
const jetSyncService_1 = require("./jetSyncService");
const syncCancellation_1 = require("../utils/syncCancellation");
const MAX_LOGS = 1000;
class JetSyncJobService {
    jobs = new Map();
    requesters = new Map();
    ...
    startJob(companyId, userId, filters, requester) {
        ...
        this.pushLog(job, 'info', 'Sincronizacao JET iniciada em segundo plano.');
    ...
    async run(job, filters) {
        try {
            const result = await jetSyncService_1.jetSyncService.executeSync(job.companyId, filters, {
                onStart: ({ total }) => {
                    job.total = total;
                    this.touch(job);
                    this.pushLog(job, 'info', `${total} idOrder(s) na fila da JET.`);
                },
                onStatusStart: ({ status, index, total }) => {
                    job.currentOrderNumber = status;
                    this.touch(job);
                    this.pushLog(job, 'info', `Processando idOrder "${status}" (${index}/${total}).`);
                },
                onStatusFinish: ({ status, index, imported, total }) => {
                    job.processed = index;
                    job.currentOrderNumber = null;
                    this.touch(job);
                    this.pushLog(job, 'success', `idOrder "${status}" concluido com ${imported} consulta(s) (${index}/${total}).`);
                },
                onLog: (message) => {
            ...
            job.error =
                error instanceof Error
                    ? error.message
                    : 'Erro desconhecido na sincronizacao da JET.';
            this.touch(job);
...
exports.jetSyncJobService = new JetSyncJobService();
