...
exports.clearOrdersDatabase = exports.cancelSyncAllOrders = exports.getSyncAllStatus = exports.startSyncAllOrders = exports.syncAllOrders = exports.syncSingleOrder = exports.searchExternalOrder = exports.openOrderTracking = exports.unarchiveOrder = exports.archiveOrder = exports.createCustomOrderStatus = exports.listCustomOrderStatuses = exports.markOrderDeliveredManually = exports.updateOrderManualData = exports.updateOrderFreightType = exports.getOrderById = exports.getOrders = exports.importOrders = void 0;
...
const correiosTrackingService_1 = require("../services/correiosTrackingService");
const sswTrackingService_1 = require("../services/sswTrackingService");
...
const orderDates_1 = require("../utils/orderDates");
const orderDb_1 = require("../lib/orderDb");
const trackingService = new trackingService_1.TrackingService();
const DEMO_SYNC_DISABLED_MESSAGE = 'Sincronizacao desabilitada para empresa demonstrativa.';
const NON_ADMIN_MANUAL_GENERAL_SYNC_LIMIT_MESSAGE = 'Sincronizacao geral de rastreio limitada a 1 execucao simultanea para usuarios nao administradores.';
const isAdminUser = (user) => String(user?.role || '')
    .trim()
    .toUpperCase() === 'ADMIN';
const hasReachedManualGeneralSyncLimit = (user) => !isAdminUser(user) && syncJobService_1.syncJobService.getRunningJobsCount() >= 1;
const safeString = (value) => {
    if (value === null || value === undefined || value === '')
        return null;
    return String(value).trim();
};
const resolveJetOrderId = (order) => {
    const rawPayload = order.apiRawPayload;
    if (!rawPayload || typeof rawPayload !== 'object')
        return null;
    return (safeString(rawPayload.jetOrderId) ||
        safeString(rawPayload.jetMeta?.idOrder) ||
        safeString(rawPayload.jetMeta?.jetOrderId) ||
        null);
};
const resolveArchivedFilterMode = (value) => {
...
const splitDocumentFields = (value) => {
...
const mapTrackingEventsToHistory = (trackingEvents) => {
...
const parseCarrierForecastFromTrackingText = (text) => {
...
const isActiveDelayedByDate = (status, targetDate) => {
    if (!targetDate)
        return false;
...
const resolveCarrierEstimatedDateFromTrackingEvents = (trackingEvents) => {
    if (!Array.isArray(trackingEvents))
        return null;
...
const getMovementDate = (order) => {
    const latestTrackingEvent = Array.isArray(order.trackingEvents)
        ? order.trackingEvents[0]
        : null;
    return (latestTrackingEvent?.eventDate ||
        order.shippingDate ||
        (0, orderDates_1.resolvePlatformCreatedDate)(order) ||
        order.lastUpdate);
};
...
const looksLikeCorreiosObjectCode = (value) => /^[A-Z]{2}\d{9}[A-Z]{2}$/.test(normalizeAlphaNumeric(value));
const shouldExcludeOrderFromPlatform = (_order) => false;
const buildSswTrackingUrl = (identifier, cnpj) => cnpj
    ? `https://ssw.inf.br/app/tracking/${cnpj}/${identifier}`
    : `https://ssw.inf.br/app/tracking/${identifier}`;
const buildIntelipostTrackingUrl = (intelipostClientId, orderNumber) => {
...
const buildCorreiosTrackingUrl = (trackingCode) => correiosTrackingService_1.correiosTrackingService.buildTrackingUrl(trackingCode);
const isCorreiosTrackingUrl = (value) => /rastreamento\.correios\.com\.br/i.test(String(value || ''));
const getStoredTrackingUrl = (order) => safeString(order.apiRawPayload?.manualTrackingUrl) ||
    ...
    null;
const hasIntelipostWebhookUpdate = (order) => Boolean(order.apiRawPayload?.intelipostWebhook);
...
const resolveTrackingSourceLabel = (order, sswRequireCnpjs = [], intelipostClientId, correiosIntegrationEnabled = true) => {
    const rawPayload = order.apiRawPayload;
    ...
    if (correiosIntegrationEnabled &&
        (source === 'CORREIOS' ||
            correiosTrackingService_1.correiosTrackingService.shouldUseForCarrier(order.freightType))) {
        const correiosTrackingUrl = buildCorreiosTrackingUrl(order.trackingCode);
        if (correiosTrackingUrl) {
            return 'Correios';
        }
    }
    if (source === 'INTELIPOST' ||
        rawPayload?.tracking ||
    ...
    const resolvedTrackingUrl = resolveOrderTrackingUrl(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled);
    ...
    if (/rastreamento\.correios\.com\.br/i.test(String(resolvedTrackingUrl || '')) &&
        buildCorreiosTrackingUrl(order.trackingCode)) {
        return 'Correios';
    }
    if (/ondeestameupedido\.com/i.test(String(resolvedTrackingUrl || '')) &&
        buildIntelipostTrackingUrl(intelipostClientId, order.orderNumber)) {
        return 'Intelipost';
    }
    return null;
};
const extractQuoteCarrierName = (quoteDetails) => {
    ...
    const isGenericServiceLabel = (value) => {
        const normalized = normalizeComparableText(value);
        if (!normalized)
            return false;
        return [
    ...
    const pickCarrierCandidate = (...candidates) => {
        for (const candidate of candidates) {
            ...
            if (!normalized)
                continue;
            if (isIntegratorLabel(normalized))
                continue;
            if (isGenericServiceLabel(normalized))
                continue;
            return normalized;
        }
        return null;
    };
    return ((quoteDetails.matchedByCarrier ? safeString(quoteDetails.requestedCarrier) : null) ||
        pickCarrierCandidate(quoteDetails.selectedCarrierName, quoteDetails.selectedOption?.carrier_name, quoteDetails.selectedOption?.carrier, quoteDetails.selectedOption?.transportadora, quoteDetails.selectedOption?.shipping_company, quoteDetails.selectedOption?.delivery_method?.carrier_name, quoteDetails.selectedOption?.taxe?.name, quoteDetails.raw?.taxe?.name, quoteDetails.carrierName, quoteDetails.shipmentType, quoteDetails.selectedOption?.delivery_method?.name, quoteDetails.selectedServiceName, quoteDetails.raw?.service_name, quoteDetails.selectedOption?.service_name, quoteDetails.selectedOption?.service, quoteDetails.selectedOption?.identifier, quoteDetails.selectedOption?.name, quoteDetails.raw?.name, quoteDetails.serviceName, quoteDetails.serviceCode) ||
        null);
};
const extractOrderQuotationId = (order) => safeString(order.originalQuotedFreightQuotationId) ||
    safeString(order.apiRawPayload?.id_quotation) ||
    safeString(order.apiRawPayload?.quotation_id) ||
    null;
const mergeQuoteDetails = (primary, fallback) => {
    if (!primary && !fallback) {
        return null;
    }
    if (!primary) {
...
const loadCheckoutQuotesMap = async (companyId, orders) => {
    const quotationIds = Array.from(new Set(orders
        .map((order) => extractOrderQuotationId(order))
        .filter((value) => Boolean(value))));
    if (!companyId || quotationIds.length === 0) {
        return new Map();
    }
    const rows = await (0, orderDb_1.listTrayCheckoutQuotesByQuotationIds)(companyId, quotationIds).catch(() => []);
    return new Map(rows.map((row) => [String(row.quotationId), row]));
};
const loadCheckoutQuoteForOrder = async (companyId, order) => {
    const quotationId = extractOrderQuotationId(order);
    if (!companyId || !quotationId) {
        return null;
    }
    return (0, orderDb_1.findTrayCheckoutQuoteByQuotationId)(companyId, quotationId).catch(() => null);
};
const resolveOrderTrackingUrl = (order, sswRequireCnpjs = [], intelipostClientId, correiosIntegrationEnabled = true) => {
    ...
    const correiosTrackingUrl = correiosIntegrationEnabled &&
        correiosTrackingService_1.correiosTrackingService.shouldUseForCarrier(order.freightType)
        ? buildCorreiosTrackingUrl(order.trackingCode)
        : null;
    if (correiosTrackingUrl) {
    ...
    const intelipostTrackingUrl = safeString(order.apiRawPayload?.source)?.toUpperCase() === 'INTELIPOST' ||
...
const resolveVerifiedOrderTrackingUrl = async (order, sswRequireCnpjs = [], intelipostClientId, correiosIntegrationEnabled = true) => {
    const intelipostTrackingUrlForWebhookOrder = hasIntelipostWebhookUpdate(order)
        ? buildIntelipostTrackingUrl(intelipostClientId, order.orderNumber)
        : null;
    if (intelipostTrackingUrlForWebhookOrder) {
    ...
    const canUseStoredTrackingUrl = storedTrackingUrl &&
        (!isCorreiosTrackingUrl(storedTrackingUrl) || correiosIntegrationEnabled)
        ? storedTrackingUrl
        : null;
    ...
    const acceptedSswUrls = [];
    for (const candidate of standardCandidates) {
        for (const cnpj of sswRequireCnpjs) {
            const result = await sswTrackingService_1.sswTrackingService.fetchTrackingByInvoice(cnpj, candidate.identifier);
            if (!result) {
                continue;
            }
    ...
    if (!invoiceIdentifier && trackingKey && isXmlTrackingKey(trackingKey)) {
        const xmlResult = await sswTrackingService_1.sswTrackingService.fetchTrackingByKey(trackingKey);
        if (xmlResult) {
            const match = shouldValidateAgainstOrder
                ? (0, sswTrackingService_1.matchSswTrackingToOrder)(order, xmlResult)
                : null;
    ...
    const intelipostTrackingUrl = safeString(order.apiRawPayload?.source)?.toUpperCase() === "INTELIPOST" ||
        ...
        ? buildIntelipostTrackingUrl(intelipostClientId, order.orderNumber)
        : null;
    return (canUseStoredTrackingUrl ||
        safeString(order.apiRawPayload?.logistic_provider?.live_tracking_url) ||
        safeString(order.apiRawPayload?.tracking_url) ||
        intelipostTrackingUrl ||
        null);
};
const getCompanySswRequireCnpjs = async (companyId) => {
    const settings = await (0, orderDb_1.getCompanyTrackingSettings)(companyId);
    if (!settings.sswRequireEnabled) {
        return [];
    }
    return Array.isArray(settings.sswRequireCnpjs)
        ? settings.sswRequireCnpjs
            .map((cnpj) => normalizeDigits(cnpj))
            .filter(Boolean)
        : [];
};
const getCompanyIntelipostClientId = async (companyId) => {
    const settings = await (0, orderDb_1.getCompanyTrackingSettings)(companyId);
...
const getCompanyCorreiosIntegrationEnabled = async (companyId) => {
    const settings = await (0, orderDb_1.getCompanyTrackingSettings)(companyId);
...
const mapIntelipostStatusToEnum = (status) => {
    ...
    if (normalizedStatus.includes('CRIADO') ||
...
const buildExternalOrderResponse = (identifier, source, result, intelipostClientId) => {
    if (source === 'SSW') {
        ...
        const recipientName = safeString(result.matchMetadata?.recipientName) ||
            safeString(result.matchMetadata?.deliveredToName) ||
            'Consulta externa';
        const recipientDocument = safeString(result.matchMetadata?.deliveredToDocument) ||
        ...
        return {
            ...
            trackingUrl,
            trackingSourceLabel: lookupMode === 'XML_KEY'
                ? 'SSW com Codigo XML'
            ...
            complement: null,
            neighborhood: '',
            city: safeString(result.matchMetadata?.destinationCity) ||
                latestEvent?.city ||
                '',
            state: safeString(result.matchMetadata?.destinationState) ||
                latestEvent?.state ||
                '',
            zipCode: '',
            totalValue: 0,
    ...
    const trackingHistory = (result?.tracking?.history || []).map((historyItem) => {
    ...
    const latestTrackingEvent = trackingHistory.length > 0
        ? trackingHistory.reduce((latest, current) => new Date(latest.date).getTime() > new Date(current.date).getTime()
            ? latest
            : current)
        : null;
    ...
    const trackingUrl = buildIntelipostTrackingUrl(intelipostClientId, resolvedOrderNumber);
    return {
...
const formatOrderForResponse = (order, sswRequireCnpjs = [], intelipostClientId, correiosIntegrationEnabled = true, checkoutQuote) => {
    const carrierEstimatedDeliveryDate = resolveCarrierEstimatedDateFromTrackingEvents(order.trackingEvents) ||
    ...
    const originalQuotedFreightValue = order.originalQuotedFreightValue ??
        checkoutQuote?.quotedValue ??
        legacyQuotedValue;
    const originalQuotedFreightDate = order.originalQuotedFreightDate ??
        checkoutQuote?.createdAt ??
        legacyQuotedDate;
    const originalQuotedFreightDetails = mergeQuoteDetails(mergeQuoteDetails(order.originalQuotedFreightDetails, checkoutQuote?.snapshotData), legacyQuotedDetails);
    const originalQuotedFreightQuotationId = extractOrderQuotationId(order) || safeString(checkoutQuote?.quotationId);
    ...
    const isPlatformDelayed = isActiveDelayedByDate(order.status, order.estimatedDeliveryDate);
    const originalQuotedCarrierName = extractQuoteCarrierName(originalQuotedFreightDetails);
    const recalculatedQuotedCarrierName = extractQuoteCarrierName(recalculatedFreightDetails);
    const freightCarrierMatchesOriginalQuote = originalQuotedCarrierName && order.freightType
        ? normalizeComparableText(order.freightType) ===
            normalizeComparableText(originalQuotedCarrierName)
        : null;
    const freightCarrierMatchesRecalculatedQuote = recalculatedQuotedCarrierName && order.freightType
        ? normalizeComparableText(order.freightType) ===
    ...
    return {
        ...
        status: order.status,
        platformCreatedAt: (0, orderDates_1.resolvePlatformCreatedDate)(order),
        carrierEstimatedDeliveryDate,
        trackingSourceLabel: resolveTrackingSourceLabel(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled),
        ...
        trackingUrl: resolveOrderTrackingUrl(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled),
    };
};
const formatOrderListResponse = (order) => {
    const carrierEstimatedDeliveryDate = resolveCarrierEstimatedDateFromTrackingEvents(order.trackingEvents) ||
        order.carrierEstimatedDeliveryDate ||
        null;
    const quotedFreightValue = order.quotedFreightValue ?? null;
    const originalQuotedFreightValue = order.originalQuotedFreightValue ?? quotedFreightValue;
    const originalQuotedFreightDate = order.originalQuotedFreightDate ?? order.quotedFreightDate ?? null;
    const recalculatedFreightValue = order.recalculatedFreightValue ?? null;
    const recalculatedFreightDate = order.recalculatedFreightDate ?? null;
    return {
        ...order,
        ...
        isPlatformDelayed: isActiveDelayedByDate(order.status, order.estimatedDeliveryDate),
        freightCarrierMatchesQuote: null,
...
const importOrders = async (req, res) => {
    ...
    try {
        ...
        const { message, results } = await (0, orderImportService_1.importOrdersForCompany)(user.companyId, orders);
        return res.json({
            success: true,
            message,
            results,
...
const getOrders = async (req, res) => {
    try {
        ...
        const orders = await (0, orderDb_1.listOrdersForCompany)(user.companyId, archivedMode);
...
const getOrderById = async (req, res) => {
    try {
        ...
        const order = await (0, orderDb_1.getOrderById)(id);
        if (!order) {
        ...
        const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(order.companyId);
        const checkoutQuote = await loadCheckoutQuoteForOrder(order.companyId, order);
        return res.json(formatOrderForResponse(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled, checkoutQuote));
    }
    catch (error) {
...
const updateOrderFreightType = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updatedOrderBase = await (0, orderDb_1.updateOrderFreightTypeById)(id, freightType);
        const updatedOrder = updatedOrderBase
            ? await (0, orderDb_1.getOrderById)(String(updatedOrderBase.id))
            : null;
        if (!updatedOrder) {
            return res.status(404).json({ error: 'Pedido nao encontrado' });
        }
        ...
        const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(user.companyId);
        ...
        return res.json({
            ...
            order: formatOrderForResponse(updatedOrder, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled, checkoutQuote),
        });
    }
    catch (error) {
        console.error('Erro ao atualizar transportadora do pedido:', error);
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao atualizar transportadora do pedido'),
        });
    }
};
exports.updateOrderFreightType = updateOrderFreightType;
const updateOrderManualData = async (req, res) => {
    try {
        const { id } = req.params;
        // @ts-ignore
        const user = req.user;
        ...
        const updatedOrderBase = await (0, orderDb_1.updateOrderManualFieldsById)({
            id,
            customerName,
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao atualizar dados do pedido'),
        });
    }
};
exports.updateOrderManualData = updateOrderManualData;
const markOrderDeliveredManually = async (req, res) => {
    try {
        const { id } = req.params;
        // @ts-ignore
        const user = req.user;
        ...
        const updatedOrderBase = await (0, orderDb_1.markOrderDeliveredManuallyById)({
            id,
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao marcar pedido como entregue'),
        });
    }
};
exports.markOrderDeliveredManually = markOrderDeliveredManually;
const listCustomOrderStatuses = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        if (!user?.companyId) {
        ...
        const rows = await (0, orderDb_1.listCompanyCustomOrderStatuses)(user.companyId);
        return res.json({
            success: true,
            statuses: rows,
        });
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao listar status personalizados'),
        });
    }
};
exports.listCustomOrderStatuses = listCustomOrderStatuses;
const createCustomOrderStatus = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        if (!user?.companyId) {
        ...
        const created = await (0, orderDb_1.upsertCompanyCustomOrderStatus)({
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao criar status personalizado'),
        });
    }
};
exports.createCustomOrderStatus = createCustomOrderStatus;
const setOrderArchivedState = async (req, res, archived) => {
    try {
        const { id } = req.params;
        // @ts-ignore
        const user = req.user;
        ...
        const updatedOrderBase = await (0, orderDb_1.updateOrderArchivedStateById)(id, archived);
        const updatedOrder = updatedOrderBase
            ? await (0, orderDb_1.getOrderById)(String(updatedOrderBase.id))
            : null;
        if (!updatedOrder) {
        ...
        if (archived) {
            await (0, orderDb_1.deleteMonitoredOrderByCompanyAndOrder)(user.companyId, id);
        }
        const sswRequireCnpjs = await getCompanySswRequireCnpjs(user.companyId);
        const intelipostClientId = await getCompanyIntelipostClientId(user.companyId);
        const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(user.companyId);
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao atualizar arquivo do pedido'),
        });
    }
};
const archiveOrder = async (req, res) => setOrderArchivedState(req, res, true);
exports.archiveOrder = archiveOrder;
const unarchiveOrder = async (req, res) => setOrderArchivedState(req, res, false);
exports.unarchiveOrder = unarchiveOrder;
const openOrderTracking = async (req, res) => {
    try {
        const { id } = req.params;
        // @ts-ignore
        const user = req.user;
        ...
        const intelipostClientId = await getCompanyIntelipostClientId(user.companyId);
        const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(user.companyId);
        const trackingUrl = await resolveVerifiedOrderTrackingUrl(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled);
        if (!trackingUrl) {
...
exports.openOrderTracking = openOrderTracking;
const searchExternalOrder = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        ...
        const existingOrder = await (0, orderDb_1.findActiveOrderByIdentifier)({
            companyId: user.companyId,
            identifier,
            normalizedDigits,
        ...
        if (existingOrder && !shouldExcludeOrderFromPlatform(existingOrder)) {
            ...
            if (syncResult.change) {
                await notificationService_1.notificationService
                    ...
                    console.error('Falha ao registrar notificacao monitorada na busca externa local:', notificationError);
                });
            }
            const refreshedOrder = await (0, orderDb_1.getOrderById)(existingOrder.id);
            const sswRequireCnpjs = await getCompanySswRequireCnpjs(user.companyId);
            const intelipostClientId = await getCompanyIntelipostClientId(user.companyId);
            const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(user.companyId);
            return res.json({
                ...
                order: refreshedOrder
                    ? formatOrderForResponse(refreshedOrder, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled)
                    : null,
            });
        }
        const externalResult = await trackingService.searchExternalIdentifier(identifier, user.companyId);
        if (!externalResult) {
            if (isCorreiosIdentifier) {
                return res.status(404).json({
                    error: 'Nao encontrei rastreio nos Correios para este codigo de objeto. Verifique o codigo informado e se a integracao dos Correios esta habilitada para a empresa ativa.',
                });
            }
            return res.status(404).json({
        ...
        const intelipostClientId = await getCompanyIntelipostClientId(user.companyId);
        const externalOrderPayload = buildExternalOrderResponse(identifier, externalResult.source, externalResult.result, intelipostClientId);
        await (0, orderImportService_1.importOrdersForCompany)(user.companyId, [externalOrderPayload]);
        const savedOrder = await (0, orderDb_1.findLatestOrderByNumberForCompany)(user.companyId, String(externalOrderPayload.orderNumber));
        const sswRequireCnpjs = await getCompanySswRequireCnpjs(user.companyId);
        const correiosIntegrationEnabled = await getCompanyCorreiosIntegrationEnabled(user.companyId);
        ...
        return res.json({
            ...
            order: savedOrder
                ? formatOrderForResponse(savedOrder, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled, checkoutQuote)
...
exports.searchExternalOrder = searchExternalOrder;
const syncSingleOrder = async (req, res) => {
    try {
        const { id } = req.params;
        // @ts-ignore
        const user = req.user;
        ...
        if (await (0, demoCompanyService_1.isDemoCompanyById)(user.companyId)) {
        ...
        if (result.change) {
            await notificationService_1.notificationService
                ...
                .catch((notificationError) => {
                console.error('Falha ao registrar notificacao monitorada no sync unitario:', notificationError);
            });
        }
        const order = await (0, orderDb_1.getOrderById)(id);
        ...
        return res.json({
            ...
            message: result.message,
            order: order
                ? formatOrderForResponse(order, sswRequireCnpjs, intelipostClientId, correiosIntegrationEnabled, checkoutQuote)
                : null,
        });
    }
    catch (error) {
        ...
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao sincronizar pedido'),
        });
    }
};
...
const syncAllOrders = async (req, res) => {
    try {
        ...
        syncJobService_1.syncJobService.ensureSchedule(user.companyId, user.id);
        let report = null;
        try {
            report = await syncReportService_1.syncReportService.sendTrackingSyncReport({
                companyId: user.companyId,
                userId: user.id,
        ...
        try {
            ...
            console.error('Falha ao registrar notificacoes da sincronizacao direta:', notificationError);
        }
        return res.json({
            success: true,
            ...
            results,
            report,
            schedule: syncJobService_1.syncJobService.getSchedule(user.companyId),
        });
    }
    catch (error) {
        console.error('Erro ao sincronizar todos os pedidos:', error);
        return res.status(500).json({
            error: (0, prismaError_1.toUserFacingDatabaseErrorMessage)(error, 'Erro ao sincronizar pedidos'),
        });
    }
};
exports.syncAllOrders = syncAllOrders;
const startSyncAllOrders = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        if (!user || !user.companyId) {
        ...
        const existingJob = syncJobService_1.syncJobService.getJob(user.companyId);
        ...
        const job = syncJobService_1.syncJobService.startJob(user.companyId, user.id, 'manual', {
            email: user.email,
            name: user.email,
        });
        return res.json({
...
exports.startSyncAllOrders = startSyncAllOrders;
const getSyncAllStatus = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        if (!user || !user.companyId) {
        ...
        if (await (0, demoCompanyService_1.isDemoCompanyById)(user.companyId)) {
            return res.json({
                success: true,
                job: null,
                schedule: syncJobService_1.syncJobService.getDisabledSchedule(),
            });
        }
        syncJobService_1.syncJobService.ensureSchedule(user.companyId, user.id);
        return res.json({
            success: true,
            job: syncJobService_1.syncJobService.getJob(user.companyId),
            schedule: syncJobService_1.syncJobService.getSchedule(user.companyId),
        });
    }
    catch (error) {
...
const cancelSyncAllOrders = async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        if (!user || !user.companyId) {
        ...
        const job = syncJobService_1.syncJobService.cancelJob(user.companyId);
        return res.json({
            success: true,
            message: job
...
exports.cancelSyncAllOrders = cancelSyncAllOrders;
const CLEANUP_SCAN_BATCH_SIZE = 1000;
const CLEANUP_DELETE_BATCH_SIZE = 1000;
const resolveCleanupPeriodWindow = ({ period, customStartDate, customEndDate, }) => {
    if (period === 'ALL') {
...
const resolveOrderCleanupReferenceDate = (order) => {
    const latestTrackingDate = safeDate(order?.trackingEvents?.[0]?.eventDate || order?.latestTrackingEventDate);
...
const matchesCleanupWindow = (referenceDate, window) => {
    if (window.type === 'ALL') {
        return true;
    }
    if (!referenceDate) {
    ...
    return (referenceDate.getTime() >= window.startDate.getTime() &&
        referenceDate.getTime() <= window.endDate.getTime());
};
const collectOrderIdsForCleanup = async ({ companyId, status, window, }) => {
    if (window.type === 'ALL') {
        return null;
    }
    const orderIds = [];
    let cursor;
    while (true) {
        const rows = await (0, orderDb_1.listOrderCleanupBatch)({
            companyId,
            status,
            cursor,
...
const deleteOrdersByIdBatches = async (orderIds) => {
    ...
    for (let index = 0; index < orderIds.length; index += CLEANUP_DELETE_BATCH_SIZE) {
        ...
        deletedCount += await (0, orderDb_1.deleteOrdersByIds)(idsBatch);
    }
    return deletedCount;
};
const clearOrdersDatabase = async (req, res) => {
    try {
        const { type, password, companyId, status, period, customStartDate, customEndDate, } = req.body;
        ...
        const company = await (0, orderDb_1.getCompanyBasicById)(companyId);
        if (!company) {
            return res.status(404).json({ error: 'Empresa nao encontrada' });
        }
        if (type === 'ALL') {
            const deletedCount = await (0, orderDb_1.deleteOrdersByCompany)(company.id);
            return res.json({
        ...
        if (type === 'DELIVERED_7_DAYS') {
            ...
            const eligibleOrderIds = (await collectOrderIdsForCleanup({
        ...
        if (type === 'FILTERED') {
            ...
            if (periodWindow.window.type === 'ALL') {
                deletedCount = await (0, orderDb_1.deleteOrdersByCompanyAndStatus)(company.id, whereClause.status || null);
            }
            else {
                const eligibleOrderIds = (await collectOrderIdsForCleanup({
                    companyId: company.id,
            ...
            const statusLabel = status && status !== 'ALL' ? ` com status ${status}` : '';
...
exports.clearOrdersDatabase = clearOrdersDatabase;
