...
const demoCompanyByIdCache = new Map<
  string,
  { isDemo: boolean; expiresAt: number }
>();
...
export const ensureDemoCompanyData = async () => {
  let companyResult = await dbQuery<any>(
    `
      SELECT
        c."id",
        c."name",
  ...
  if (!company) {
    ...
    const createdResult = await dbQuery<any>(
      `
        INSERT INTO "Company" (
          "id",
          "name",
        ...
        RETURNING *
      `,
      [createdId, DEMO_COMPANY_NAME, DEMO_COMPANY_CNPJ],
    );
    ...
    const updatedResult = await dbQuery<any>(
      `
        UPDATE "Company" c
        SET
          "cnpj" = $2,
