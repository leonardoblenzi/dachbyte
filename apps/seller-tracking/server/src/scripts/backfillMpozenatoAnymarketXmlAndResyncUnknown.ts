...
const isAnymarketUnknownTracking = (order: Pick<OrderRow, 'latestTrackingStatus' | 'latestTrackingDescription'>) => {
...
async function main() {
  ...
  console.log(
    `Filtro UNKNOWN: ${ONLY_UNKNOWN_ORDERS ? 'SIM' : 'NAO'} | Batch limit: ${BATCH_LIMIT > 0 ? BATCH_LIMIT : 'sem limite'} | Batch offset: ${BATCH_OFFSET > 0 ? BATCH_OFFSET : 0}`,
  );
  ...
  if (orders.length > 0) {
    console.log(
      `Faixa deste lote: ${orders[0].orderNumber} -> ${orders[orders.length - 1].orderNumber}`,
    );
  }
  ...
  for (const order of orders) {
    ...
    try {
      ...
      const shouldSyncOrder =
        ONLY_UNKNOWN_ORDERS || isAnymarketUnknownTracking(order);
...
main().catch((error) => {
  console.error(
    'Falha no script de backfill AnyMarket XML + resync UNKNOWN:',
    formatError(error),
  );
  process.exitCode = 1;
});
