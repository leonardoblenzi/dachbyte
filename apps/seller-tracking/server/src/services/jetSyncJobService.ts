...
import { jetSyncService, type JetSyncFiltersInput } from './jetSyncService';
import {
...
class JetSyncJobService {
  ...
  private async run(job: SyncJobStatus, filters: JetSyncFiltersInput) {
    try {
      const result = await jetSyncService.executeSync(job.companyId, filters, {
        onStart: ({ total }) => {
          job.total = total;
          this.touch(job);
        ...
        onStatusStart: ({ status, index, total }) => {
          ...
          this.pushLog(
            job,
            'info',
            `Processando idOrder "${status}" (${index}/${total}).`,
          );
        },
        onStatusFinish: ({ status, index, imported, total }) => {
          ...
          this.pushLog(
            job,
            'success',
            `idOrder "${status}" concluido com ${imported} consulta(s) (${index}/${total}).`,
          );
        },
        onLog: (message) => {
...
export const jetSyncJobService = new JetSyncJobService();
