...
async function main() {
    ...
    const result = await anymarketSyncService_1.anymarketSyncService.executeSync(company.id, {
        days: DAYS,
    ...
    console.log(`Resumo: criados=${result.results.created} atualizados=${result.results.updated} ignorados=${result.results.skipped} eventos=${result.results.totalTrackingEvents} erros=${result.results.errors.length}`);
}
main().catch((error) => {
