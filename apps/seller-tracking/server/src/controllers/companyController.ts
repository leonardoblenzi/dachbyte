...
const normalizeIntegrationAutoSyncStatuses = (value: unknown) => {
  if (!Array.isArray(value)) return [];
...
const normalizeShippingCutoffTime = (value: unknown) => {
  if (value === undefined) return undefined;
...
const parseDatabaseHost = (databaseUrl: unknown) => {
...
const requireAdminUser = (req: Request, res: Response) => {
  if (!req.user) {
...
const mapCompanyRowToResponse = (row: any) =>
  sanitizeCompanyResponse({
    id: row.id,
    name: row.name,
    cnpj: row.cnpj,
...
export const createCompany = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const normalizedTenantGlobalId =
      normalizeOptionalString(tenantGlobalId) || crypto.randomUUID();
    ...
    const jetIntegrationEnabled = normalizeBooleanSetting(
      req.body?.jetIntegrationEnabled,
    );
    const jetOrderLookupUrlTemplate = normalizeOptionalString(
      req.body?.jetOrderLookupUrlTemplate,
    );
    const jetIntegrationKeyRaw = req.body?.jetIntegrationKey;
    const jetIntegrationKey =
      jetIntegrationKeyRaw === undefined
        ? undefined
    ...
    const jetUsername =
      jetUsernameRaw === undefined ? undefined : normalizeOptionalString(jetUsernameRaw);
    const jetPasswordRaw = req.body?.jetPassword;
    const jetPassword =
      jetPasswordRaw === undefined ? undefined : normalizeOptionalString(jetPasswordRaw);
    const jetBearerTokenRaw = req.body?.jetBearerToken;
    const jetBearerToken =
      jetBearerTokenRaw === undefined
        ? undefined
    ...
    if (jetIntegrationEnabled === true && !effectiveJetApiKey) {
      return res.status(400).json({
        error:
          'A Credencial de Integracao JET (apiKey) precisa ser informada para ativar a integracao.',
      });
    }

    const now = new Date();
    const companyId = crypto.randomUUID();
    const shippingCutoffTime = normalizeShippingCutoffTime(
      req.body?.shippingCutoffTime,
    );
...
export const updateCurrentCompanyIntegration = async (
  ...
  try {
    ...
    const intelipostApiKey =
      intelipostApiKeyRaw === undefined || intelipostApiKeyRaw === null
        ? undefined
    ...
    const integrationAutoSyncStatuses =
      integrationAutoSyncStatusesRaw === undefined
        ? undefined
    ...
    const currentCompanyResult = await dbQuery<any>(
      `
        SELECT
          "trayIntegrationEnabled",
          "anymarketIntegrationEnabled",
    ...
    const shouldAutoEnableAnymarket =
      anymarketIntegrationEnabled === undefined &&
      anymarketToken !== undefined &&
      Boolean(anymarketToken);

    const effectiveMagazordApiBaseUrl =
      magazordApiBaseUrl !== undefined
        ? magazordApiBaseUrl
        : (currentCompany as any).magazordApiBaseUrl;
    const effectiveMagazordApiUser =
      magazordApiUser !== undefined
        ? magazordApiUser
        : (currentCompany as any).magazordApiUser;
    const shouldAutoEnableMagazord =
      magazordIntegrationEnabled === undefined &&
      (magazordApiBaseUrl !== undefined ||
        magazordApiUser !== undefined ||
    ...
    const effectiveJetApiKey =
      jetIntegrationKey !== undefined
        ? jetIntegrationKey
        : (currentCompany as any).jetIntegrationKey;
    ...
    if ((willEnableJet || jetIntegrationKey !== undefined) && !effectiveJetApiKey) {
      return res.status(400).json({
        error:
          'A Credencial de Integracao JET (apiKey) precisa ser informada para ativar ou configurar a integracao.',
      });
    }
    ...
    if (erpIntegrationData.anymarketIntegrationEnabled !== undefined)
      pushUpdate(
        'anymarketIntegrationEnabled',
        erpIntegrationData.anymarketIntegrationEnabled,
      );
    ...
    if (erpIntegrationData.magazordIntegrationEnabled !== undefined)
      pushUpdate(
        'magazordIntegrationEnabled',
        erpIntegrationData.magazordIntegrationEnabled,
      );
    ...
    if (sswRequireEnabled !== undefined) pushUpdate('sswRequireEnabled', sswRequireEnabled);
    if (correiosIntegrationEnabled !== undefined)
      pushUpdate('correiosIntegrationEnabled', correiosIntegrationEnabled);
    if (anymarketToken !== undefined) pushUpdate('anymarketToken', anymarketToken);
    if (magazordApiBaseUrl !== undefined)
      pushUpdate('magazordApiBaseUrl', magazordApiBaseUrl);
    if (magazordApiUser !== undefined) pushUpdate('magazordApiUser', magazordApiUser);
    ...
    if (jetIntegrationKey !== undefined) pushUpdate('jetIntegrationKey', jetIntegrationKey);
    if (jetStoreId !== undefined) pushUpdate('jetStoreId', jetStoreId);
    if (jetUsername !== undefined) pushUpdate('jetUsername', jetUsername);
    if (jetPassword !== undefined) pushUpdate('jetPassword', jetPassword);
    if (jetBearerToken !== undefined) pushUpdate('jetBearerToken', jetBearerToken);
    if (sswRequireCnpjs !== undefined) pushUpdate('sswRequireCnpjs', sswRequireCnpjs);
...
export const getCompanyUsageSummary = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const trackingSyncRowsResult = await dbQuery<{
      createdAt: Date;
      payload: unknown;
    }>(
      `
    ...
    let lastTrackingSyncAt: string | null = null;
    ...
    return res.json({
      ...
      usage: {
        ...
        usageScore:
          Number(company.ordersCount || 0) + trackingSyncRunsLast30Days,
      },
      lastTrackingSyncAt,
    });
  } catch (error) {
