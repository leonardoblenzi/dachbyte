...
const { decrypt } = require("../security/tokenVault");

function attachIds(rows) {
  return (rows || []).map((row) => ({ id: crypto.randomUUID(), ...row }));
}

class MetaAdsSyncRepository {
    ...
    return withTenant(this.pool, job.tenant_id, async (client) => {
      const { rows } = await client.query(
        `SELECT a.id AS ad_account_id, a.workspace_id, a.connection_id, a.external_account_id,
                a.name, a.currency_code, a.timezone, a.sync_enabled, a.last_synced_at,
                c.status AS connection_status,
                cr.access_token_ciphertext, cr.access_token_expires_at, cr.scope
         ...
         WHERE a.tenant_id = $1 AND a.id = $2 AND a.provider = 'meta_ads' LIMIT 1`,
        [job.tenant_id, job.ad_account_id],
      );
      const row = rows[0];
      if (!row) return null;
      return { ...row, accessToken: decrypt(row.access_token_ciphertext) };
    });
  }
  ...
  async startRun(job, syncType) {
    ...
    await withTenant(this.pool, job.tenant_id, (client) => client.query(
      `INSERT INTO ads_sync_runs
       ...
       VALUES ($1, $2, $3, 'meta_ads', $4, 'running', now(), $5::jsonb)`,
  ...
  async finishRun(job, runId, { success, error, cursor, syncIntervalMinutes }) {
    await withTenant(this.pool, job.tenant_id, async (client) => {
      await client.query(
        ...
        [
          job.tenant_id, job.workspace_id, runId, success ? "succeeded" : "failed",
      ...
      await client.query(
        `UPDATE ads_ad_accounts
         SET last_synced_at = CASE WHEN $3 THEN now() ELSE last_synced_at END,
             last_sync_status = $4, last_sync_error = $5, updated_at = now()
    ...
    await this.pool.query(
      `UPDATE ads_sync_jobs
       SET locked_at = NULL, locked_by = NULL, last_error = $2,
           ...
           updated_at = now()
       WHERE id = $1`,
      [job.id, error ? String(error.message || error).slice(0, 2000) : null, String(success ? syncIntervalMinutes : retryMinutes), success],
    );
  }

  async dropJob(jobId) { await this.pool.query("DELETE FROM ads_sync_jobs WHERE id = $1", [jobId]); }
  ...
  async persistSnapshot({ tenantId, workspaceId, adAccountId, data, window }) {
    return withTenant(this.pool, tenantId, async (client) => {
      ...
      await this.upsertCreatives(client, tenantId, workspaceId, adAccountId, data.creatives || []);
      ...
      await this.upsertActionMetrics(client, tenantId, workspaceId, adAccountId, data.actionMetrics || []);
      await client.query(
        `INSERT INTO ads_sync_cursors
           (tenant_id, workspace_id, ad_account_id, provider, dataset, cursor, last_success_at, last_attempt_at)
         VALUES ($1, $2, $3, 'meta_ads', 'core', $4::jsonb, now(), now())
         ON CONFLICT (tenant_id, ad_account_id, provider, dataset) DO UPDATE SET
           cursor = EXCLUDED.cursor, last_success_at = now(), last_attempt_at = now(), updated_at = now()`,
  ...
  async upsertCampaigns(client, tenantId, workspaceId, adAccountId, rows) {
    ...
    await client.query(
      `WITH input AS (
       ...
       SELECT id, $1, $2, $3, 'meta_ads', external_campaign_id, name, status,
              channel_type, bidding_strategy_type, COALESCE(metadata, '{}'::jsonb)
       FROM input
  ...
  async upsertAdGroups(client, tenantId, workspaceId, adAccountId, rows) {
    ...
    await client.query(
      `WITH input AS (
       ...
       SELECT i.id, $1, $2, $3, c.id, 'meta_ads', i.external_ad_group_id,
  ...
  async upsertCreatives(client, tenantId, workspaceId, adAccountId, rows) {
    if (!rows.length) return;
    const payload = attachIds(rows);
    await client.query(
      `WITH input AS (
         SELECT * FROM jsonb_to_recordset($4::jsonb) AS x(
           id uuid, external_creative_id text, name text, title text, body text,
           thumbnail_url text, image_url text, call_to_action_type text, metadata jsonb
         )
       )
       INSERT INTO ads_meta_creatives
         (id, tenant_id, workspace_id, ad_account_id, external_creative_id, name,
          title, body, thumbnail_url, image_url, call_to_action_type, metadata)
       SELECT id, $1, $2, $3, external_creative_id, name, title, body,
              thumbnail_url, image_url, call_to_action_type, COALESCE(metadata, '{}'::jsonb)
       FROM input
       ON CONFLICT (tenant_id, ad_account_id, external_creative_id) DO UPDATE SET
         ...
         metadata = ads_meta_creatives.metadata || EXCLUDED.metadata, updated_at = now()`,
      [tenantId, workspaceId, adAccountId, JSON.stringify(payload)],
    );
  }
  ...
  async upsertAds(client, tenantId, workspaceId, adAccountId, rows) {
    ...
    await client.query(
      `WITH input AS (
       ...
       SELECT i.id, $1, $2, $3, c.id, g.id, 'meta_ads', i.external_ad_id,
    ...
    await client.query(
      `WITH input AS (
         SELECT * FROM jsonb_to_recordset($4::jsonb) AS x(
           ...
           conversions numeric, conversion_value numeric, all_conversions numeric, all_conversion_value numeric,
           reach bigint, unique_clicks bigint, frequency numeric, link_clicks bigint, outbound_clicks bigint,
           metadata jsonb
         )
       )
       INSERT INTO ads_metrics_daily
         (tenant_id, workspace_id, ad_account_id, provider, metric_date, entity_type,
          ...
          all_conversions, all_conversion_value, reach, unique_clicks, frequency,
          link_clicks, outbound_clicks, metadata)
       SELECT $1, $2, $3, 'meta_ads', metric_date, entity_type, entity_external_id,
              campaign_external_id, ad_group_external_id, COALESCE(impressions,0), COALESCE(clicks,0),
              COALESCE(cost_micros,0), COALESCE(conversions,0), COALESCE(conversion_value,0),
              COALESCE(all_conversions,0), COALESCE(all_conversion_value,0), COALESCE(reach,0),
              COALESCE(unique_clicks,0), COALESCE(frequency,0), COALESCE(link_clicks,0),
              COALESCE(outbound_clicks,0), COALESCE(metadata, '{}'::jsonb)
       FROM input
       ON CONFLICT (tenant_id, ad_account_id, metric_date, entity_type, entity_external_id) DO UPDATE SET
         ...
         all_conversions = EXCLUDED.all_conversions, all_conversion_value = EXCLUDED.all_conversion_value,
         reach = EXCLUDED.reach, unique_clicks = EXCLUDED.unique_clicks, frequency = EXCLUDED.frequency,
         link_clicks = EXCLUDED.link_clicks, outbound_clicks = EXCLUDED.outbound_clicks,
         metadata = ads_metrics_daily.metadata || EXCLUDED.metadata, updated_at = now()`,
      [tenantId, workspaceId, adAccountId, JSON.stringify(rows)],
    );
  }

  async upsertActionMetrics(client, tenantId, workspaceId, adAccountId, rows) {
    if (!rows.length) return;
    await client.query(
      `WITH input AS (
         SELECT * FROM jsonb_to_recordset($4::jsonb) AS x(
           metric_date date, entity_type text, entity_external_id text,
           action_type text, action_count numeric, action_value numeric
         )
       )
       INSERT INTO ads_meta_action_metrics_daily
         (tenant_id, workspace_id, ad_account_id, metric_date, entity_type,
          entity_external_id, action_type, action_count, action_value)
       SELECT $1, $2, $3, metric_date, entity_type, entity_external_id,
              action_type, COALESCE(action_count,0), COALESCE(action_value,0)
       FROM input
       ON CONFLICT (tenant_id, ad_account_id, metric_date, entity_type, entity_external_id, action_type) DO UPDATE SET
         action_count = EXCLUDED.action_count, action_value = EXCLUDED.action_value, updated_at = now()`,
      [tenantId, workspaceId, adAccountId, JSON.stringify(rows)],
    );
  }
}

module.exports = { MetaAdsSyncRepository };
