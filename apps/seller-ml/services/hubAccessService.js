...
function buildIdentity(input) {
  ...
  if (!company?.empresa_id) {
    if (isMaster) {
      return { ok: true, bypass: true, reason: "master_bypass_without_company" };
    }
  ...
  return {
    ...
    payload: {
      ...
      document_type: normalizeDocumentType(company.document_type),
      document_number: normalizeDocumentNumber(company.document_number),
      user_id: userId,
      full_name: fullName,
      email,
      role: String(input?.role || user?.nivel || "owner").toLowerCase(),
      module: input?.module || undefined,
      modules: Array.isArray(input?.modules) ? input.modules : undefined,
      access_policy: input?.access_policy || undefined,
    },
  };
}
