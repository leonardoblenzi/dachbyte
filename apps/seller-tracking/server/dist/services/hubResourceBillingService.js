...
exports.buildTrackingResourceContext = exports.HubResourceBillingError = exports.checkTrackingResourceAccess = exports.syncTrackingResource = void 0;
...
exports.HubResourceBillingError = HubResourceBillingError;
...
const billingMode = () => {
    const value = String(process.env.HUB_RESOURCE_BILLING_MODE || 'monitor')
        .trim()
        .toLowerCase();
    if (['off', 'disabled', 'legacy'].includes(value))
        return 'off';
    if (['strict', 'enforce', 'enabled'].includes(value))
        return 'enforce';
    return 'monitor';
};
const hubConfig = () => ({
...
const requestHub = async (method, pathname, payload = null) => {
...
const responseError = (response, fallbackMessage) => {
...
const buildTrackingResourceContext = (input) => {
...
exports.buildTrackingResourceContext = buildTrackingResourceContext;
const syncTrackingResource = async (input) => {
    const mode = billingMode();
    if (mode === 'off')
        return { bypass: true, reason: 'billing_off' };
    ...
    try {
        ...
        if (!response.ok)
            throw responseError(response, 'Nao foi possivel sincronizar o Rastreio com o Hub.');
...
exports.syncTrackingResource = syncTrackingResource;
const checkTrackingResourceAccess = async (input) => {
    const mode = billingMode();
    if (mode === 'off')
        return { allow: true, reason: 'billing_off' };
    const context = buildTrackingResourceContext(input);
    try {
        await (0, exports.syncTrackingResource)(input);
        const response = await requestHub('POST', '/v1/internal/resources/access', {
            module_slug: MODULE_SLUG,
            account_id: context.accountId,
        });
        if (!response.ok)
            throw responseError(response, 'Nao foi possivel validar acesso do Rastreio no Hub.');
...
exports.checkTrackingResourceAccess = checkTrackingResourceAccess;
