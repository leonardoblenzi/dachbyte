...
const listTargetOrderNumbers = async (companyId) => {
    const result = await (0, db_1.dbQuery)(`
...
async function main() {
    ...
    for (const orderNumber of orderNumbers) {
        try {
            ...
            processed += 1;
            console.log(`  OK ${orderNumber}: criado=${importResult.results.created} atualizado=${importResult.results.updated} ignorado=${importResult.results.skipped}`);
        }
        catch (error) {
            errors += 1;
