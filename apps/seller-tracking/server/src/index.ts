...
import { getModSituation, shouldRunAutomaticProcesses } from './utils/modSituation';
import { startHubUsageReporter } from './services/hubUsageReporter';
  ...
  try {
    ...
    if (trackingResult.handled && trackingResult.text) {
      return res.json({
        ...
        actions: Array.isArray((trackingResult as any).actions)
          ? (trackingResult as any).actions
          : [],
      });
    }
    ...
    if (structuredResult.handled && structuredResult.text) {
      return res.json({
        ...
        actions: Array.isArray((structuredResult as any).actions)
          ? (structuredResult as any).actions
          : [],
      });
    }
...
app.get("/api/health", (req, res) => {
  res.json({
    ok: true,
    modSituation: getModSituation(),
    automaticProcessesEnabled: shouldRunAutomaticProcesses(),
  });
});
...
export const createAvantrackingApp = () => {
  startHubUsageReporter();
  return app;
};

export const initializeAvantrackingSchedules = async () => {
  if (!shouldRunAutomaticProcesses()) {
    console.log(
      `[AUTO] Processos automaticos desabilitados por MOD_SITUATION=${getModSituation()}.`,
    );
    return;
  }

  const initializers = [
    ['rastreio', () => syncJobService.initializeSchedules()],
    ['pedidos Tray', () => traySyncJobService.initializeSchedules()],
    ['pedidos ANYMARKET', () => anymarketSyncJobService.initializeSchedules()],
    ['saude das integracoes', () => integrationHealthService.initialize()],
    ['relatorio semanal', () => weeklyMovementReportService.initializeSchedule()],
    ['relatorio mensal', () => monthlyMovementReportService.initializeSchedule()],
  ] as const;
...
export const startAvantrackingServer = async () => {
  ...
  try {
    await initializeAvantrackingSchedules();
  } catch (error) {
