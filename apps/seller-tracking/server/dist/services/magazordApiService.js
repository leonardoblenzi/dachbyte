...
const toMagazordDateFilter = (value) => {
    if (!value) {
        return undefined;
    }
...
const parseOrderStatusCode = (value) => {
    const parsed = Number(String(value ?? '').trim());
    return Number.isFinite(parsed) ? parsed : null;
};
const unwrapOrderPayload = (value) => {
    if (value && typeof value === 'object' && !Array.isArray(value) && value.data) {
        return value.data;
    }
    return value;
};
const extractOrderCode = (value) => pickFirstString(value?.codigo, value?.codigoPedido, value?.idPedido, value?.id, value?.codigoMarketplace);
...
const normalizeRastreioItems = (value) => {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.map((item) => unwrapOrderPayload(item)).filter(Boolean);
};
const collectValuesByKeys = (node, targetKeys, maxDepth = 6, depth = 0, visited = new WeakSet()) => {
    if (!node || depth > maxDepth) {
        return [];
    }
    if (Array.isArray(node)) {
        return node.flatMap((item) => collectValuesByKeys(item, targetKeys, maxDepth, depth + 1, visited));
    }
    ...
    for (const [key, value] of Object.entries(node)) {
        ...
        if (value && typeof value === 'object') {
            values.push(...collectValuesByKeys(value, targetKeys, maxDepth, depth + 1, visited));
        }
    }
    return values;
};
const findFirstStringByKeys = (node, keys, options) => {
    const normalizedKeys = new Set(keys.map((key) => normalizeComparableText(key).replace(/[^a-z0-9]/g, '')));
...
const findFirstDateByKeys = (node, keys) => {
    const normalizedKeys = new Set(keys.map((key) => normalizeComparableText(key).replace(/[^a-z0-9]/g, '')));
    const values = collectValuesByKeys(node, normalizedKeys);
    for (const value of values) {
        const parsed = safeDate(value);
...
const projectDateFromDays = (baseDate, days) => {
    if (!baseDate || !days) {
        return null;
    }
...
const isLikelyCarrierName = (value) => {
...
const extractTrackingCodeFromOrder = (order) => {
    ...
    for (const rastreio of fromRastreioItems) {
        const code = pickFirstString(rastreio?.codigoRastreio, rastreio?.codigo, rastreio?.trackingCode, rastreio?.rastreio, rastreio?.pedidoRastreioCodigo, rastreio?.codigoEnvio, rastreio?.codigoObjeto, rastreio?.objeto, rastreio?.pedras_codigo_rastreio);
        if (code) {
            return code;
        }
    }
    return pickFirstString(order?.pedidoTrackingCodigo, order?.codigoRastreio, order?.codigoEnvio, order?.codigoObjeto, parsedTrackingParams?.codigoRastreio, parsedTrackingParams?.codigo, parsedTrackingParams?.trackingCode);
};
const extractTrackingUrlFromOrder = (order) => {
    const parsedTrackingParams = tryParseJsonObject(order?.pedidoTrackingParams);
    const fromRastreioItems = normalizeRastreioItems(order?.arrayPedidoRastreio);
    for (const rastreio of fromRastreioItems) {
        const url = pickFirstString(rastreio?.url, rastreio?.link, rastreio?.linkRastreio, rastreio?.trackingUrl, rastreio?.urlRastreio, rastreio?.urlTracking, rastreio?.pedidoRastreioUrl, rastreio?.pedras_link_rastreio);
        if (url) {
            return url;
        }
    }
    return pickFirstString(order?.trackingUrl, order?.pedidoTrackingUrl, order?.urlRastreio, order?.linkRastreio, parsedTrackingParams?.url, parsedTrackingParams?.link, parsedTrackingParams?.trackingUrl);
};
const extractInvoiceNumberFromOrder = (order) => {
    const direct = pickFirstString(order?.numeroNotaFiscal, order?.notaFiscalNumero, order?.nfeNumero, order?.nfNumero, order?.numeroNf, order?.notaFiscalChave, order?.chaveNfe, order?.chaveAcessoNfe);
    if (direct) {
        return direct;
    }
...
const extractFreightTypeFromOrder = (order) => {
    const parsedTrackingParams = tryParseJsonObject(order?.pedidoTrackingParams);
    const fromRastreioItems = normalizeRastreioItems(order?.arrayPedidoRastreio);
    for (const rastreio of fromRastreioItems) {
        const candidate = pickFirstString(rastreio?.transportadoraNome, rastreio?.transportadora, rastreio?.nomeTransportadora, rastreio?.transportadoraServicoNome, rastreio?.transportadoraServico, rastreio?.servicoTransportadora, rastreio?.carrierName, rastreio?.carrier);
        if (candidate && isLikelyCarrierName(candidate)) {
            return candidate;
        }
    }
    const direct = pickFirstString(order?.transportadoraNome, order?.transportadora, order?.nomeTransportadora, order?.transportadoraServicoNome, order?.transportadoraServico, parsedTrackingParams?.transportadoraNome, parsedTrackingParams?.transportadora, parsedTrackingParams?.carrierName);
    if (direct && isLikelyCarrierName(direct)) {
        return direct;
    }
    const recursive = findFirstStringByKeys(order, [
        'transportadoraNome',
        'transportadora',
        'nomeTransportadora',
...
const extractEstimatedDeliveryDateFromOrder = (order) => {
    const parsedTrackingParams = tryParseJsonObject(order?.pedidoTrackingParams);
    const fromRastreioItems = normalizeRastreioItems(order?.arrayPedidoRastreio);
    const direct = safeDate(order?.pedidoTrackingPrevisaoEntrega) ||
    ...
    for (const rastreio of fromRastreioItems) {
        const fromTracking = safeDate(rastreio?.dataPrevisaoEntrega) ||
    ...
    const nestedDate = findFirstDateByKeys(parsedTrackingParams, [
        'dataPrevisaoEntrega',
        'previsaoEntrega',
    ...
    const prazoDias = parsePositiveDays(order?.prazoEntrega) ||
...
const mapMagazordStatusToInternal = (statusCode, statusText) => {
    ...
    if (statusCode !== null) {
        ...
        if (statusCode === 7)
            return 'SHIPPED';
        if (statusCode === 6 || statusCode === 15 || statusCode === 23)
            return 'CREATED';
        if (canceledStatuses.has(statusCode))
            return 'CANCELED';
        if (returnedStatuses.has(statusCode))
            return 'RETURNED';
        if (failureStatuses.has(statusCode))
            return 'FAILURE';
    }
    ...
    if (normalized.includes('CANCEL'))
        return 'CANCELED';
    if (normalized.includes('DEVOLV') || normalized.includes('REVERSA'))
        return 'RETURNED';
    if (normalized.includes('FRAUDE') ||
...
const buildTrackingHistory = (order, mappedStatus, city, state) => {
    const sourceHistory = Array.isArray(order?.arrayPedidoRastreio)
        ? order.arrayPedidoRastreio
        : [];
    const fallbackDate = safeDate(order?.dataHoraUltimaAlteracaoSituacao) ||
        safeDate(order?.dataHora) ||
        new Date();
    if (sourceHistory.length > 0) {
        return sourceHistory.map((event) => {
            const eventStatusCode = parseOrderStatusCode(event?.pedidoRastreioSituacao || event?.situacao);
            const eventStatusText = pickFirstString(event?.pedidoRastreioSituacaoDescricao, event?.situacaoDescricao, event?.descricao);
            return {
...
const resolveCustomerDocument = (documentValue) => {
...
class MagazordApiService {
    ...
    async getOrderTrackingByCode(orderCode) {
        const { client } = await this.getClient();
        try {
            const response = await client.get(`/v2/site/pedido/${encodeURIComponent(orderCode)}/rastreio`);
    ...
    async loadDetailedOrders(orders, hooks) {
        ...
        for (let start = 0; start < entries.length; start += DETAIL_BATCH_SIZE) {
            ...
            const chunkResult = await Promise.all(chunk.map(async (entry) => {
                try {
                    ...
                    hooks?.onLog?.(`Falha ao carregar detalhe do pedido ${entry.code} na Magazord; usando dados resumidos da listagem.`);
    ...
    async syncAllOrders(params, hooks) {
        ...
        while (true) {
            ...
            hooks?.onLog?.(`Buscando pagina ${page} da Magazord${params.situacao ? ` para situacao "${Array.isArray(params.situacao) ? params.situacao.join(',') : params.situacao}"` : ''}.`);
    ...
    mapMagazordOrderToSystem(magazordOrder) {
        const order = unwrapOrderPayload(magazordOrder);
        const document = resolveCustomerDocument(order?.pessoaCpfCnpj);
        const pedidoSituacaoCodigo = parseOrderStatusCode(order?.pedidoSituacao ?? order?.situacao);
        const pedidoSituacaoDescricao = pickFirstString(order?.pedidoSituacaoDescricao, order?.pedidoSituacaoDescricaoDetalhada, order?.situacaoDescricao);
        const mappedStatus = mapMagazordStatusToInternal(pedidoSituacaoCodigo, pedidoSituacaoDescricao);
        const city = pickFirstString(order?.cidadeNome, order?.cidade);
        const state = pickFirstString(order?.estadoSigla, order?.estado);
        const shippingDate = safeDate(order?.dataHoraUltimaAlteracaoSituacao) ||
            safeDate(order?.dataHora);
        ...
        const orderNumber = pickFirstString(order?.codigo, order?.id, order?.codigoMarketplace);
        ...
        const invoiceAccessKey = pickFirstString(order?.notaFiscalChave, order?.notaFiscalChaveAcesso, order?.chaveAcessoNfe, order?.nfeChave, order?.chaveAcesso);
        const invoiceXmlUrl = pickFirstString(order?.notaFiscalXmlUrl, order?.xmlNotaFiscalUrl, order?.urlXmlNotaFiscal, order?.linkXmlNotaFiscal);
        ...
        return {
            ...
            cpf: document.cpf,
            cnpj: document.cnpj,
            phone: pickFirstString(order?.pessoaTelefone, order?.telefone),
            mobile: pickFirstString(order?.pessoaCelular, order?.pessoaTelefone),
