...
import { intelipostWebhookService } from '../services/intelipostWebhookService';
import { anymarketWebhookService } from '../services/anymarketWebhookService';
import { dbQuery } from '../lib/db';

const readApiKeyFromRequest = (req: Request) => {
...
export const receiveIntelipostWebhook = async (req: Request, res: Response) => {
  try {
...
export const listIntelipostWebhookLogs = async (req: Request, res: Response) => {
  try {
    ...
    const logs = await intelipostWebhookService.listWebhookLogs(companyId, limit);
...
export const listAnymarketWebhookLogs = async (req: Request, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) {
      return res.status(403).json({ error: 'Acesso negado. Usuario sem empresa.' });
    ...
    const limitRaw = Number(req.query?.limit || 100);
    const limit = Number.isFinite(limitRaw) ? limitRaw : 100;
    const logs = await anymarketWebhookService.listWebhookLogs(companyId, limit);
...
export const clearWebhookFailureLogs = async (req: Request, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) {
      return res.status(403).json({ error: 'Acesso negado. Usuario sem empresa.' });
    ...
    const result =
      provider === 'intelipost'
        ? await intelipostWebhookService.clearFailureLogs(companyId)
        : await anymarketWebhookService.clearFailureLogs(companyId);
...
export const reprocessWebhookFailureLogs = async (req: Request, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) {
      return res.status(403).json({ error: 'Acesso negado. Usuario sem empresa.' });
    ...
    const result =
      provider === 'intelipost'
        ? await intelipostWebhookService.reprocessFailureLogs(companyId, limit)
        : await anymarketWebhookService.reprocessFailureLogs(companyId, limit);
...
export const receiveAnymarketWebhook = async (req: Request, res: Response) => {
  try {
    const result = await anymarketWebhookService.processWebhook({
      payload: req.body || {},
...
export const listMonitoringFailureAcks = async (req: Request, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) {
      return res.status(403).json({ error: 'Acesso negado. Usuario sem empresa.' });
...
export const acknowledgeMonitoringFailureAlert = async (req: Request, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) {
      return res.status(403).json({ error: 'Acesso negado. Usuario sem empresa.' });
