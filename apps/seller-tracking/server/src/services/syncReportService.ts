...
const summarizeByLabel = (labels: string[]) => {
...
const normalizeUpdateLabel = (label: string) => {
...
const normalizeSkippedReason = (reason: string) => {
...
const summarizeUpdatedOrders = (orders: TraySyncOrderReport[]) =>
  summarizeByLabel(
    orders.flatMap((order) =>
      Array.isArray(order.updateSummary)
        ? order.updateSummary.map((entry) => normalizeUpdateLabel(entry))
        : [],
    ),
  );

const summarizeSkippedOrders = (orders: TraySyncSkippedOrderReport[]) =>
  summarizeByLabel(
    orders.map((order) => normalizeSkippedReason(order.reason)),
  );

const buildSummaryListHtml = (
  ...
  const emptyMessage =
    options?.emptyMessage || 'Sem ocorrencias para esta sincronizacao.';
  ...
  return `
    <ul style="margin:${marginTop}px 0 0;padding-left:18px;font-size:12px;line-height:1.55;color:${listColor};">
      ${items
        ...
        .map(
          (item) =>
            `<li>${escapeHtml(item.label)}: <strong>${item.count}</strong> pedido(s)</li>`,
        )
        .join('')}
    </ul>
  `;
...
const buildTraySkippedRowsHtml = (orders: TraySyncSkippedOrderReport[]) => {
  if (orders.length === 0) {
    return `
...
const buildTrayOrderRowsHtml = (orders: TraySyncOrderReport[], title: string) => {
  ...
  const hasUpdateSummary = orders.some(
    (order) => Array.isArray(order.updateSummary) && order.updateSummary.length > 0,
  );
...
const buildTraySyncEmailText = (
  ...
  const updatedSummaryText = buildSummaryText(
    summarizeUpdatedOrders(payload.updatedOrders),
    'Nenhuma alteracao em pedidos.',
  );
  const skippedSummaryText = buildSummaryText(
    summarizeSkippedOrders(skippedOrders),
    'Nenhum pedido ignorado.',
  );
...
class SyncReportService {
  private async resolveRecipients(input: {
    ...
    if (uniqueRecipients.size === 0 && input.userId) {
      const triggeringUserResult = await dbQuery<{
        email: string;
        name: string | null;
        receivePlatformEmails: boolean | null;
      }>(
  ...
  private async getCompanyName(companyId: string) {
    const result = await dbQuery<{ name: string | null }>(
      `
        SELECT "name"
        FROM "Company"
        WHERE "id" = $1
